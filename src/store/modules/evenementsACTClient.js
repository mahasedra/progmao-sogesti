...
const collName = 'evenementsACTClient';
const evenementsACTClient = {
    namespaced: true,
    state: {
        evenementsACTClient: [],
    ...
    mutations: {
        SET_evenementsACT(state, evenementsACTClient) {
            state.evenementsACTClient = evenementsACTClient;
        },
        ADD_evenementsACT(state, order) {
            state.evenementsACTClient.push(order);
        },
        UPDATE_ORDER(state, payload) {
            Object.assign(state.evenementsACTClient[payload.index], payload.order);
        },
        REMOVE_evenementsACT(state, order) {
            const index = state.evenementsACTClient.indexOf(order);
            state.evenementsACTClient.splice(index, 1);
        }
    },
    ...
    actions: {
        async loadevenementsACT({ commit }) {
            ...
            let order;
            let evenementsACTClient = querySnapshot.docs.map(function (doc) {
                order = doc.data();
                order.id = doc.id;
                return order;
            });
            commit('SET_evenementsACT', evenementsACTClient);
        },
        ...
        async addevenementsACT({ commit }, order) {
            ...
            await firestore()
                ...
                .get().then((snapshot) => {
                    ...
                    console.log(documents[0]['evenementsClientCount'])
                    actualId = documents[0]['evenementsClientCount'] + 1;
            ...
            audit["event"] = 'Événements ACT Client';
                    ...
                    for (let i = 0; i < documents.length; i++) {
                        let body = {
                            ...
                            'notification': {
                                ...
                                'body': 'Bonjour Mr/Mme ' +
                                    documents[i]['nom'] +
                                    "\nNouveau formulaire 'Incidents Client' soumis par " + role + " " + name + '.\nID: ' + 'INCL' + actualId + '\nDate et heure: ' +
                                    dateString,
                            },
            ...
            await firestore().collection("stats").doc("D2zmmdQjpRwFtHunf1zo").update({
                'evenementsClientCount': firebase.firestore.FieldValue.increment(1),
            });
        },
...
export default evenementsACTClient;