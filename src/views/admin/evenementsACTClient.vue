...
<v-card class="spacing-playground ma-5">
    <v-data-table :options="{ openLink }" :headers="headers" :items="evenementsACTClient" :search="search" class="elevation-1" :loading="loading" loading-text="Chargement...Veuillez patienter">
        <template v-slot:top>
            <v-toolbar flat>
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Intitulé</h4>
                                            <v-text-field outlined prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.intitule" label="Intitulé" :rules="titleRule"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                    ...
                                    <v-row>
                                        <v-col cols="20">
                                            <v-textarea filled prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.commentaires" label="Commantaires de l'agent">
                                            </v-textarea>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                            ...
                            <v-row>
                                <v-col cols="12">
                                    ...
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Intitulé</h4>
                                            <v-text-field outlined prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.intitule" label="Intitulé" :rules="titleRule"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
        ...
        <template v-slot:[`item.status`]="{ value }">
            ...
            <v-chip v-if="value != 'Approuvé' && value != 'Non approuvé' && value != 'Validé' && value != 'Non validé' && value != 'En cours'" color="grey" style="color: aliceblue">{{ value }}</v-chip>
        </template>
        <template v-slot:[`item.idForm`]="{ item }">
            <v-chip color="blue" class="ma-2" outlined style="color: aliceblue" @click="editItem(item)">{{ item.idForm }}
            </v-chip>
        </template>
...
export default {
    ...
    methods: {
        ...
        async exportFormToPDF(idForm) {
            ...
            doc.text("Commentaire de l'agent: " + documents[0]["commentaires"], 10, 185, {
                align: "left",
            });
        ...
        exportPDF() {
            ...
            import("jspdf").then(jsPDF => {
                import("jspdf-autotable").then(() => {
                    ...
                    doc.setFontSize(16);
                    doc.setFont("helvetica");
                    doc.text("Liste des incidents déclarés par les clients", 30, 30, {
                        align: "justify",
                    });
                    doc.autoTable(exportColumns, this.evenementsACTClient);
                    doc.save("Liste des incidents déclarés par les clients.pdf");
                    this.loading2 = false;
                })
            });
        },
        async exportToExcel() {
            ...
            const snapshot = await firestore().collection("evenementsACTClient").get();
            ...
            workbook.xlsx.writeBuffer().then((data) => {
                ...
                fs.saveAs(blob, `Liste des incidents déclarés par les clients.xlsx`);
            });
            this.loading1 = false;
        },
        async initialize() {
