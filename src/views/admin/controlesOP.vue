<template>
<v-card class="spacing-playground ma-5">
    <v-data-table :options="{ openLink }" :headers="headers" :items="controlesOP" :search="search" class="elevation-4" :loading="loading" loading-text="Chargement...Veuillez patienter">
        <template v-slot:top>
            <v-toolbar flat>
                <v-toolbar-title class="underline">Liste</v-toolbar-title>
                <v-spacer />

                <v-dialog v-model="dialog" max-width="700px">
                    <base-material-card>
                        <template v-slot:heading>
                            <div class="display-2 font-weight-light">
                                {{ formTitle }}
                            </div>

                            <div class="subtitle-1 font-weight-light">
                                Completer les informations
                            </div>
                        </template>
                        <v-form v-bind:disabled="loading" lazy-validation ref="dialogForm">
                            <v-row>
                                <v-col cols="12">
                                    <v-select prepend-inner-icon="mdi-apps" label="Status" :disabled="loading" v-model="editedItem.status" :items="items2"></v-select>
                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12">
                                    <h4>Votre commentaire</h4>
                                    <vue-editor v-model="editedItem.commentaires" label="Commentaires de l'admin"></vue-editor>

                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12" class="text-right">
                                    <v-btn :disabled="loading" class="success" color="" text @click="save">Enregistrer</v-btn>
                                    <v-btn :disabled="loading" color="" text @click="close">Quitter</v-btn>
                                </v-col>
                            </v-row>
                        </v-form>
                    </base-material-card>
                </v-dialog>

                <v-dialog v-model="dialogAdd" max-width="700px">
                    <base-material-card>
                        <template v-slot:heading>
                            <div class="display-2 font-weight-light">
                                Ajouter un contrôle
                            </div>

                            <div class="subtitle-1 font-weight-light">
                                Completer les informations
                            </div>
                        </template>
                        <v-form v-bind:disabled="loading" lazy-validation ref="dialogForm">
                            <v-row>
                                <v-col cols="12">
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Intitulé</h4>
                                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline" v-model="defaultItem.titleControleOp" label="Titre du contrôle" :rules="titleRule">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>

                                    <center>
                                        <h2 style="font-weight: bold">LVP 1</h2>
                                        <v-row>
                                            <v-col cols="12">
                                                <v-combobox :disabled="loading" v-model="defaultItem.lvp1" :items="items4" dense label="LVP 1"></v-combobox>
                                            </v-col>
                                        </v-row>
                                    </center>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.feuBalisesAllume" label="Feux et balises de piste allumés">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.informationTransmises" label="Information transmise aux différents intervenants (SALT, ST Handling, Aérotransports, Pétrolier, Lomé catering, Aviation générale)">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-textarea prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.firstSectionObs" label="Observations">
                                            </v-textarea>
                                        </v-col>
                                    </v-row>

                                    <v-divider height="20px"></v-divider>
                                    <br />
                                    <center>
                                        <h2 style="font-weight: bold">LVP 2</h2>
                                        <v-row>
                                            <v-col cols="12">
                                                <v-combobox :disabled="loading" v-model="defaultItem.lvp2" :items="items4" dense label="LVP 2"></v-combobox>

                                            </v-col>
                                        </v-row>
                                    </center>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.feuBalisesAllume2" label="Feux et balises de piste allumés">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.informationTransmises2" label="Information transmise aux différents intervenants (SALT, ST Handling, Aérotransports, Pétrolier, Lomé catering, Aviation générale)">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="
                          defaultItem.dispositifEclairageExterieurZoneReserveEnMarche
                        " label="Dispositifs d’éclairage extérieurs des zones réservées sont mis en marche">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.respectVitesse" label="Respect des vitesses de circulation (10km/h dans les couloirs, 5km/h par heure autour des aéronefs">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="
                          defaultItem.dispositifAntiBrouillardAllume
                        " label="Dispositif antibrouillard disponible et allumé sur les engins"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.zoneTravauxZoneFermeDelimite" label="Zone de travaux et  zone fermée sont délimitées"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.balisageLumineuxAllume" label="Balisage lumineux de l’aire de trafic,  balisage des obstacles et  dispositifs d’éclairage des panneaux de signalisation et d’obligation sont allumés">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="
                          defaultItem.TravauxEntretienSuspenduEtPersonnelDegage
                        " label="Travaux d’entretien sur l’aire de trafic sont suspendus. Le personnel et les engins sont dégagés des aires">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.accesAireDeManeouvreLimite" label="Accès à l’aire de manœuvre limité uniquement aux véhicules de SLI, PRA et de maintenance ASECNA">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="
                          defaultItem.serviceControlAvertiDeToutePanne
                        " label="Services de contrôle aérien avertis de toute panne ou défaillance des aides visuelles">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-textarea prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.seocondSectionObs" label="Observations">
                                            </v-textarea>
                                        </v-col>
                                    </v-row>

                                    <v-divider height="20px"></v-divider>
                                    <br />
                                    <center>
                                        <h2 style="font-weight: bold">LVP 3</h2>
                                        <v-row>
                                            <v-col cols="12">
                                                <v-combobox :disabled="loading" v-model="defaultItem.lvp3" :items="items4" dense label="LVP 3"></v-combobox>
                                            </v-col>
                                        </v-row>
                                    </center>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.arretTouteOperation" label="Arrêt de toutes les opérations">
                                            </v-select>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" filled prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.accesAireDeManeouvreLimite2" label="Accès à l’aire de manœuvre limité uniquement aux véhicules de SLI, PRA et de maintenance ASECNA">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" filled prepend-inner-icon="mdi-format-align-justify" v-model="
                          defaultItem.serviceControlAvertiDeToutePanne2
                        " label="Services de contrôle aérien avertis de toute panne ou défaillance des aides visuelles">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" filled prepend-inner-icon="mdi-format-align-justify" v-model="
                          defaultItem.formulaireCompteRenduEvenementRensiegne
                        " label="Formulaire de compte rendu d’évènement renseigné"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-textarea prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.thirdSectionObs" label="Observations">
                                            </v-textarea>
                                        </v-col>
                                    </v-row>

                                    <v-divider height="20px"></v-divider>
                                    <br />
                                    <center>
                                        <h2 style="font-weight: bold">
                                            Retour à la situastion normale
                                        </h2>
                                    </center>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" filled prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.urpaInformeParTwr" label="UPRA informé par la TWR">
                                            </v-select>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.verificationvisuelParUrpa" label="Vérifications visuelles par UPRA">
                                            </v-select>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" filled prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.urpaInformeCommandant" label="UPRA informe le commandant de permanence de la SALT"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" filled prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.urpaInformeAutreIntervenant" label="UPRA informe les autres intervenants sur l’aire de trafic"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-textarea prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.fourthSectionObs" label="Observations">
                                            </v-textarea>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items2" prepend-inner-icon="mdi-apps" label="Status" :disabled="loading" v-model="defaultItem.status"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            Détails du statut
                                            <vue-editor v-model="defaultItem.detailStatut" label="Détails du statut"> </vue-editor>
                                        </v-col>
                                    </v-row>
                                    <v-divider height="20px"></v-divider>
                                    <br />
                                    <center>
                                        <h2 style="font-weight: bold">Entités à prévenir</h2>
                                    </center>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-checkbox v-model="defaultItem.DT" :label="`DT`"></v-checkbox>
                                            <v-checkbox v-model="defaultItem.DCGR" :label="`DCGR`"></v-checkbox>
                                            <v-checkbox v-model="defaultItem.DG" :label="`DG`"></v-checkbox>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Votre commentaire</h4>
                                            <vue-editor v-model="editedItem.commentaireAdmin"></vue-editor>

                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <input type="file" multiple class="d-none" ref="uploader" accept="image/*" @change="uploadTaskImages()" />
                                            <v-btn :disabled="loading" color="primary" v-model="imagePret" @click="$refs.uploader.click()">
                                                <v-icon dark>
                                                    mdi-image-multiple
                                                </v-icon> <b>Ajouter des images</b>
                                            </v-btn>
                                        </v-col>

                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <input type="file" multiple class="d-none" ref="uploader1" accept=".xlsx,.xls,.doc, .docx,.ppt, .pptx,.txt,.pdf" @change="uploadTaskDocuments()" />
                                            <v-btn :disabled="loading" color="red" v-model="documentPret" @click="$refs.uploader1.click()">
                                                <v-icon dark>
                                                    mdi-file-arrow-up-down
                                                </v-icon> <b>Ajouter des documents</b>
                                            </v-btn>

                                        </v-col>

                                    </v-row>
                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12" class="text-right">
                                    <v-btn :disabled="loading" class="success" color="" text @click="save">Enregistrer</v-btn>
                                    <v-btn :disabled="loading" color="" text @click="closeAdd">Quitter</v-btn>
                                </v-col>
                            </v-row>
                        </v-form>
                    </base-material-card>
                </v-dialog>

                <v-dialog v-model="dialogView" max-width="700px">
                    <base-material-card>
                        <template v-slot:heading>
                            <div class="display-2 font-weight-light">
                                Modifier un contrôle
                            </div>

                            <div class="subtitle-1 font-weight-light">
                                Completer les informations
                            </div>
                        </template>
                        <v-form v-bind:disabled="loading" lazy-validation ref="dialogForm">
                            <v-row>
                                <v-col cols="12">
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Intitulé</h4>
                                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline" v-model="editedItem.titleControleOp" label="Titre du contrôle" :rules="titleRule">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <!-- <v-row>
                          <v-col cols="12" sm="6" md="10">
                            <v-select prepend-inner-icon="mdi-apps" label="Type LVP" :disabled="loading"
                              v-model="editedItem.typeLVP" :items="items3"></v-select>
                          </v-col>
                        </v-row> -->
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline" v-model="editedItem.userName" label="Nom et prénoms" disabled="true"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <center>
                                        <h2 style="font-weight: bold">LVP 1</h2>
                                        <v-row>
                                            <v-col cols="12">
                                                <v-combobox :disabled="loading" v-model="editedItem.lvp1" :items="items4" dense label="LVP 1">
                                                </v-combobox>

                                            </v-col>
                                        </v-row>
                                    </center>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.feuBalisesAllume" label="Feux et balises de piste allumés">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.informationTransmises" label="Information transmise aux différents intervenants (SALT, ST Handling, Aérotransports, Pétrolier, Lomé catering, Aviation générale)">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-textarea v-if="editedItem.firstSectionObs != ''" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.firstSectionObs" label="Observations"></v-textarea>
                                        </v-col>
                                    </v-row>

                                    <v-divider height="20px"></v-divider>
                                    <br />
                                    <center>
                                        <h2 style="font-weight: bold">LVP 2</h2>
                                        <v-row>
                                            <v-col cols="12">
                                                <v-combobox :disabled="loading" v-model="editedItem.lvp2" :items="items4" dense label="LVP 2">
                                                </v-combobox>
                                            </v-col>
                                        </v-row>
                                    </center>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.feuBalisesAllume2" label="Feux et balises de piste allumés">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.informationTransmises2" label="Information transmise aux différents intervenants (SALT, ST Handling, Aérotransports, Pétrolier, Lomé catering, Aviation générale)">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="
                          editedItem.dispositifEclairageExterieurZoneReserveEnMarche
                        " label="Dispositifs d’éclairage extérieurs des zones réservées sont mis en marche">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.respectVitesse" label="Respect des vitesses de circulation (10km/h dans les couloirs, 5km/h par heure autour des aéronefs">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="
                          editedItem.dispositifAntiBrouillardAllume
                        " label="Dispositif antibrouillard disponible et allumé sur les engins"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.zoneTravauxZoneFermeDelimite" label="Zone de travaux et  zone fermée sont délimitées"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.balisageLumineuxAllume" label="Balisage lumineux de l’aire de trafic,  balisage des obstacles et  dispositifs d’éclairage des panneaux de signalisation et d’obligation sont allumés">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="
                          editedItem.TravauxEntretienSuspenduEtPersonnelDegage
                        " label="Travaux d’entretien sur l’aire de trafic sont suspendus. Le personnel et les engins sont dégagés des aires">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.accesAireDeManeouvreLimite" label="Accès à l’aire de manœuvre limité uniquement aux véhicules de SLI, PRA et de maintenance ASECNA">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="
                          editedItem.serviceControlAvertiDeToutePanne
                        " label="Services de contrôle aérien avertis de toute panne ou défaillance des aides visuelles">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-textarea v-if="editedItem.seocondSectionObs != ''" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.seocondSectionObs" label="Observations"></v-textarea>
                                        </v-col>
                                    </v-row>

                                    <v-divider height="20px"></v-divider>
                                    <br />
                                    <center>
                                        <h2 style="font-weight: bold">LVP 3</h2>
                                        <v-row>
                                            <v-col cols="12">
                                                <v-combobox :disabled="loading" v-model="editedItem.lvp3" :items="items4" dense label="LVP 3">
                                                </v-combobox>
                                            </v-col>
                                        </v-row>
                                    </center>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.arretTouteOperation" label="Arrêt de toutes les opérations">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field filled prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.accesAireDeManeouvreLimite2" label="Accès à l’aire de manœuvre limité uniquement aux véhicules de SLI, PRA et de maintenance ASECNA">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field filled prepend-inner-icon="mdi-format-align-justify" v-model="
                          editedItem.serviceControlAvertiDeToutePanne2
                        " label="Services de contrôle aérien avertis de toute panne ou défaillance des aides visuelles">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field filled prepend-inner-icon="mdi-format-align-justify" v-model="
                          editedItem.formulaireCompteRenduEvenementRensiegne
                        " label="Formulaire de compte rendu d’évènement renseigné"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-textarea v-if="editedItem.thirdSectionObs != ''" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.thirdSectionObs" label="Observations"></v-textarea>
                                        </v-col>
                                    </v-row>

                                    <v-divider height="20px"></v-divider>
                                    <br />
                                    <center>
                                        <h2 style="font-weight: bold">
                                            Retour à la situastion normale
                                        </h2>
                                    </center>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field filled prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.urpaInformeParTwr" label="UPRA informé par la TWR">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.verificationvisuelParUrpa" label="Vérifications visuelles par UPRA">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field filled prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.urpaInformeCommandant" label="UPRA informe le commandant de permanence de la SALT"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field filled prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.urpaInformeAutreIntervenant" label="UPRA informe les autres intervenants sur l’aire de trafic"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-textarea v-if="editedItem.fourthSectionObs != ''" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.fourthSectionObs" label="Observations"></v-textarea>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select prepend-inner-icon="mdi-apps" label="Status" :disabled="loading" v-model="editedItem.status" :items="items2"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            Détails du statut
                                            <vue-editor v-model="editedItem.detailStatut" label="Détails du statut"> </vue-editor>
                                        </v-col>
                                    </v-row>
                                    <v-divider height="20px"></v-divider>
                                    <br />
                                    <center>
                                        <h2 style="font-weight: bold">Entités à prévenir</h2>
                                    </center>
                                    <v-row>
                                        <v-col cols="12">
                                            <h3>
                                                <v-checkbox v-model="editedItem.DT" :label="`DT`"></v-checkbox>
                                                <v-checkbox v-model="editedItem.DCGR" :label="`DCGR`"></v-checkbox>
                                                <v-checkbox v-model="editedItem.DG" :label="`DG`"></v-checkbox>
                                            </h3>
                                        </v-col>
                                    </v-row>
                                    <v-divider></v-divider>
                                    <v-spacer></v-spacer>
                                    <v-col cols="12" v-if="editedItem.customFieldsData && editedItem.customFieldsData.length > 0">
                                        <h2 style="color: red; text-align: center;">Champs personnalisés</h2>
                                        <div v-for="item of editedItem.customFieldsData" v-bind:key="item.cle" v-bind:video="item">

                                            <v-row v-if="item.type == 'Zone de texte'">
                                                <v-col cols="12">
                                                    <h4>{{ item.titre }}</h4>
                                                    <vue-editor v-model="editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur" label="Commentaires de l'admin"></vue-editor>

                                                </v-col>
                                            </v-row>

                                            <v-row v-if="item.type == 'Texte' || item.type == 'Numérique'">
                                                <v-col cols="12">
                                                    <h4>{{ item.titre }}</h4>
                                                    <v-text-field prepend-inner-icon="mdi-format-align-justify" :disabled="loading" v-model="editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur">
                                                    </v-text-field>
                                                </v-col>
                                            </v-row>

                                            <v-row v-if="item.type == 'Date'">
                                                <v-col cols="12">
                                                    <h4>{{ item.titre }}</h4>

                                                    <v-text-field v-model="editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur" readonly v-bind="attrs" v-on="on"></v-text-field>

                                                </v-col>
                                            </v-row>

                                            <v-row v-if="item.type == 'Case à cocher'">
                                                <v-col cols="12">
                                                    <v-checkbox v-model="editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur" :label="item.titre"></v-checkbox>
                                                </v-col>
                                            </v-row>

                                            <v-row v-if="item.type == 'Liste déroulante'">
                                                <v-col cols="12">
                                                    <v-select prepend-inner-icon="mdi-apps" :label="item.titre" :disabled="loading" v-model="editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur" :items="item.listData"></v-select>
                                                </v-col>
                                            </v-row>

                                            <v-row v-if="item.type == 'Fichier'">
                                                <v-col cols="12">
                                                    <h4>{{ item.titre }}</h4>
                                                    <v-btn class="ma-2" color="primary" @click="() => openLink(editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur)" download>
                                                        VOIR LE FICHIER
                                                    </v-btn>
                                                </v-col>
                                            </v-row>

                                        </div>
                                    </v-col>
                                    <v-divider></v-divider>
                                    <v-spacer></v-spacer>
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Votre commentaire</h4>
                                            <!-- <v-textarea prepend-inner-icon="mdi-format-align-justify" 
                               v-model="editedItem.commentaireAdmin" label="Commentaires de l'admin">
                            </v-textarea> -->
                                            <vue-editor v-model="editedItem.commentaireAdmin" label="Commentaires de l'admin">
                                            </vue-editor>

                                        </v-col>
                                        <v-row v-if="editedItem.commentaireAdmin != '' && editedItem.commentedBy != '' && editedItem.commentedBy != null" style="color: red; font-weight:bold">
                                            Ce commentaire a été laissé par {{ editedItem.roleCommentateur }} {{ editedItem.commentedBy
                        }}
                                            à {{ editedItem.dateComment }}.
                                        </v-row>
                                    </v-row>

                                    <br>
                                    <div v-if="editedItem.imagesJoined != null && editedItem.imagesJoined.length > 0">
                                        <v-divider></v-divider>
                                        <h4> Images envoyées </h4>
                                        <v-row>
                                            <div v-for="item of editedItem.imagesJoined" v-bind:key="item.cle" v-bind:video="item">
                                                <v-avatar size="100px" class="my-2" @click="() => openLink(item)">
                                                    <img :src="item" />
                                                </v-avatar>
                                                &nbsp;&nbsp;
                                            </div>

                                        </v-row>
                                    </div>
                                    <br>
                                    <div v-if="editedItem.documentsJoined != null && editedItem.documentsJoined.length > 0">
                                        <v-divider></v-divider>
                                        <h4> Documents envoyées</h4>
                                        <v-row>
                                            <div v-for="item of editedItem.documentsJoined" v-bind:key="item.cle" v-bind:video="item">
                                                <v-btn class="ma-2" color="error" @click="() => openLink(item)" download>
                                                    <b> VOIR LE FICHIER {{ editedItem.documentsJoined.indexOf(item) + 1 }}</b>
                                                </v-btn>
                                                &nbsp;&nbsp;
                                            </div>
                                        </v-row>
                                    </div>
                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12" class="text-right">
                                    <v-btn :disabled="loading" class="success" color="" text @click="save">Enregistrer</v-btn>
                                    <v-btn :disabled="loading" color="" text @click="closeView">Quitter</v-btn>
                                </v-col>
                            </v-row>
                        </v-form>
                    </base-material-card>
                </v-dialog>

                <v-dialog v-model="dialogMail" max-width="450px">
                    <base-material-card>
                        <template v-slot:heading>
                            <div class="display-2 font-weight-light">
                                Envoyer le formulaire par email
                            </div>

                            <div class="subtitle-1 font-weight-light">
                                Completer les informations
                            </div>
                        </template>
                        <v-form v-bind:disabled="loading" lazy-validation ref="dialogForm">
                            <v-row>
                                <v-col cols="12">
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-email-outline" v-model="email" label="Email du destinataire" :rules="titleRule"></v-text-field>
                                        </v-col>
                                    </v-row>
                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12" class="text-right">
                                    <v-btn :disabled="loading" class="success" color="" text @click="sendMail">Enregistrer</v-btn>
                                    <v-btn :disabled="loading" color="" text @click="closeMail">Quitter</v-btn>
                                </v-col>
                            </v-row>
                        </v-form>
                    </base-material-card>
                </v-dialog>
                <v-spacer></v-spacer>
                <v-btn :disabled="loading1" class="mb-2 white--text py-4" @click="addItem()" color="rgba(0, 157, 160, 0.11)" depressed dark small v-bind="attrs" v-on="on">
                    <v-icon color="rgb(0, 144, 146)">
                        mdi-plus
                    </v-icon>
                </v-btn>

                <v-btn text icon class="mb-2 ml-2" @click="initialize">
                    <v-icon>mdi-refresh</v-icon>
                </v-btn>

            </v-toolbar>
            <v-divider class="mx-4" inset horizontal />

            <v-card-title>
                <v-spacer></v-spacer>

                <v-col class="d-flex align-baseline" cols="12" sm="6" md="4">
                    <v-text-field v-model="search" append-icon="mdi-magnify" label="Rechercher ici" single-line hide-details></v-text-field>

                    <v-btn outlined class="mx-2 py-4" dark small color="red" :loading="loading2" :disabled="loading2" @click="exportPDF()">
                        <v-icon color="red">
                            mdi-file-pdf-box
                        </v-icon>
                    </v-btn>

                    <v-btn outlined class="mx-2 py-4" dark small color="green" :loading="loading1" :disabled="loading1" @click="exportToExcel()">
                        <v-icon color="green">
                            mdi-cloud-upload
                        </v-icon>
                    </v-btn>
                </v-col>

            </v-card-title>

        </template>
        <template v-slot:[`item.idForm`]="{ item }">
            <v-chip color="blue" style="color: aliceblue" class="ma-2" outlined @click="viewItem(item)">{{ item.idForm }}
            </v-chip>
        </template>
        <!-- <template v-slot:[`item.createdAt`]="{ value }">
        <span class="">{{ value.toDate() }}</span>
      </template> -->
        <template v-slot:[`item.status`]="{ value }">
            <v-chip v-if="value == 'En cours'" color="green" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
            <v-chip v-if="value == 'Validé'" color="blue" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
            <v-chip v-if="value == 'Non validé'" color="red" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
            <v-chip v-if="value == 'Non approuvé'" color="red" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
            <v-chip v-if="value == 'Approuvé'" color="blue" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
            <v-chip v-if="value != 'Approuvé' && value != 'Non approuvé' && value != 'Validé' && value != 'Non validé' && value != 'En cours'" color="grey" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
        </template>
        <template v-slot:[`item.documentAccompagne`]="{ value }">
            <v-btn v-if="value != ''" class="ma-2" color="error" @click="() => openLink(value)" download>
                VOIR
            </v-btn>
        </template>

        <template v-slot:[`item.actions`]="{ item }">

            <v-menu bottom left>
                <template v-slot:activator="{ on, attrs }">

                    <v-btn class="mx-2 " outlined fab dark small color="black" v-bind="attrs" v-on="on">
                        <v-icon color="black">
                            mdi-dots-vertical
                        </v-icon>
                    </v-btn>

                </template>

                <v-list>
                    <v-list-item link>
                        <v-list-item-title @click="exportFormToPDF(item.idForm)">Exporter</v-list-item-title>
                    </v-list-item>
                    <v-list-item link>
                        <v-list-item-title @click="printForm(item.idForm)">Imprimer</v-list-item-title>
                    </v-list-item>
                    <v-list-item link>
                        <v-list-item-title @click="viewItem(item)">Editer</v-list-item-title>
                    </v-list-item>
                    <v-list-item link>
                        <v-list-item-title @click="viewMail(item)">Envoyer</v-list-item-title>
                    </v-list-item>
                    <v-list-item link>
                        <v-list-item-title @click="deleteItem(item)">Supprimer</v-list-item-title>
                    </v-list-item>
                </v-list>
            </v-menu>

        </template>

        <template v-slot:no-data>
            <v-btn color="primary" @click="initialize">Réinitialiser</v-btn>
        </template>
    </v-data-table>

    <v-snackbar v-model="snack" :timeout="3000" :color="snackColor">
        {{ snackText }}
        <template v-slot:action="{ attrs }">
            <v-btn v-bind="attrs" text @click="snack = false">Fermer</v-btn>
        </template>
    </v-snackbar>
</v-card>
</template>

<script>
import {
    mapActions,
    mapGetters
} from "vuex";
const Excel = require("exceljs");
import {
    firestore
} from "firebase";
import * as fs from "file-saver";
import {
    storage
} from "firebase";
import jsPDF from "jspdf";
import {
    VueEditor
} from "vue2-editor";
import AXIOS from "axios";

export default {
    components: {
        VueEditor
    },
    data() {
        return {

            snack: false,
            snackColor: "",
            snackText: "",
            search: "",
            loading: true,
            loading1: false,
            loading2: false,
            dialog: false,
            dialogAdd: false,
            dialogMail: false,
            email: "",
            formId: "",
            dialogView: false,
            headers: [{
                    text: "ID",
                    value: "idForm"
                },
                {
                    text: "Titre du contrôle",
                    align: "start",
                    sortable: true,
                    value: "titleControleOp",
                },
                {
                    text: "Nom et prénoms",
                    align: "start",
                    sortable: true,
                    value: "userName",
                },

                {
                    text: "Date et heure d'envoi",
                    value: "dateEnvoi"
                },

                // { text: "Commentaires", value: "commentaireAdmin" },
                {
                    text: "Statut",
                    value: "status",
                    sortable: true
                },
                {
                    text: "Actions",
                    value: "actions",
                    sortable: false
                },
            ],
            titleRule: [(v) => !!v || "Champ obligatoire"],
            items2: localStorage.getItem("role") == "Superviseur (Chef Service)" ? ["Terminé", "Validé", "Non validé", "En cours"] : ["Terminé", "Ouvert", "Ouvert", "Approuvé", "Refusé", "Réglé", "Non réglé(pièces en commande)", "Non réglé (matériel à changer)", "Non réglé (pas de contrat)", "En cours"],
            items3: ["Oui", "Non"],
            items4: [
                '600m < LVP < 1500m',
                '350m < LVP < 600m',
                'LVP < 350m',
            ],
            editedIndex: -1,
            editedItem: {
                detailStatut: "",
                commentaires: "",
                lvp1: "600m < LVP < 1500m",
                lvp2: "600m < LVP < 1500m",
                lvp3: "600m < LVP < 1500m",
                roleCommentateur: "",
                commentedBy: "",
                titleControleOp: "",
                DCGR: "",
                DG: "",
                DT: "",
                TWR: "",
                imagesJoined: "",
                documentsJoined: "",
                feuBalisesAllume: "",
                informationTransmises: "",
                feuBalisesAllume2: "",
                informationTransmises2: "",
                dispositifEclairageExterieurZoneReserveEnMarche: "",
                respectVitesse: "",
                dispositifAntiBrouillardAllume: "",
                zoneTravauxZoneFermeDelimite: "",
                balisageLumineuxAllume: "",
                accesAireDeManeouvreLimite: "",
                serviceControlAvertiDeToutePanne: "",
                arretTouteOperation: "",
                accesAireDeManeouvreLimite2: "",
                serviceControlAvertiDeToutePanne2: "",
                formulaireCompteRenduEvenementRensiegne: "",
                TravauxEntretienSuspenduEtPersonnelDegage: "",
                firstSectionObs: "",
                seocondSectionObs: "",
                thirdSectionObs: "",
                fourthSectionObs: "",
                urpaInformeParTwr: "",
                verificationvisuelParUrpa: "",
                urpaInformeCommandant: "",
                urpaInformeAutreIntervenant: "",
                userEmail: "",
                userId: "",
                userName: "",
                status: "",
                commentaireAdmin: "",
                dateEnvoi: "",
                id: "",
            },
            defaultItem: {
                detailStatut: "",
                lvp1: "600m < LVP < 1500m",
                lvp2: "600m < LVP < 1500m",
                lvp3: "600m < LVP < 1500m",
                commentaires: "",
                roleCommentateur: "",
                commentedBy: "",
                titleControleOp: "",
                DCGR: false,
                DG: false,
                DT: false,
                imagesJoined: [],
                documentsJoined: [],
                feuBalisesAllume: "",
                informationTransmises: "",
                feuBalisesAllume2: "",
                informationTransmises2: "",
                dispositifEclairageExterieurZoneReserveEnMarche: "",
                respectVitesse: "",
                dispositifAntiBrouillardAllume: "",
                zoneTravauxZoneFermeDelimite: "",
                balisageLumineuxAllume: "",
                accesAireDeManeouvreLimite: "",
                serviceControlAvertiDeToutePanne: "",
                arretTouteOperation: "",
                accesAireDeManeouvreLimite2: "",
                serviceControlAvertiDeToutePanne2: "",
                TravauxEntretienSuspenduEtPersonnelDegage: "",
                formulaireCompteRenduEvenementRensiegne: "",
                firstSectionObs: "",
                seocondSectionObs: "",
                thirdSectionObs: "",
                fourthSectionObs: "",
                urpaInformeParTwr: "",
                verificationvisuelParUrpa: "",
                urpaInformeCommandant: "",
                urpaInformeAutreIntervenant: "",
                userEmail: "",
                userId: "",
                userName: "",
                status: "En cours",
                commentaireAdmin: "",
                dateEnvoi: "",
                id: "",
            },
            defaultItemInit: {
                detailStatut: "",
                lvp1: "600m < LVP < 1500m",
                lvp2: "600m < LVP < 1500m",
                lvp3: "600m < LVP < 1500m",
                commentaires: "",
                roleCommentateur: "",
                commentedBy: "",
                titleControleOp: "",
                DCGR: false,
                DG: false,
                DT: false,
                imagesJoined: [],
                documentsJoined: [],
                feuBalisesAllume: "",
                informationTransmises: "",
                feuBalisesAllume2: "",
                informationTransmises2: "",
                dispositifEclairageExterieurZoneReserveEnMarche: "",
                respectVitesse: "",
                dispositifAntiBrouillardAllume: "",
                zoneTravauxZoneFermeDelimite: "",
                balisageLumineuxAllume: "",
                accesAireDeManeouvreLimite: "",
                serviceControlAvertiDeToutePanne: "",
                arretTouteOperation: "",
                accesAireDeManeouvreLimite2: "",
                serviceControlAvertiDeToutePanne2: "",
                TravauxEntretienSuspenduEtPersonnelDegage: "",
                formulaireCompteRenduEvenementRensiegne: "",
                firstSectionObs: "",
                seocondSectionObs: "",
                thirdSectionObs: "",
                fourthSectionObs: "",
                urpaInformeParTwr: "",
                verificationvisuelParUrpa: "",
                urpaInformeCommandant: "",
                urpaInformeAutreIntervenant: "",
                userEmail: "",
                userId: "",
                userName: "",
                status: "En cours",
                commentaireAdmin: "",
                dateEnvoi: "",
                id: "",
            },
        };
    },
    computed: {

        ...mapGetters({
            controlesOP: "controlesOP/getcontrolesOP",
        }),
        formTitle() {
            return "Modifier le statut";
        },
    },

    watch: {
        dialog(val) {
            val || this.close();
        },
        dialogView(val) {
            val || this.close();
        },
    },

    created() {
        this.initialize();
    },

    methods: {
        //mail sending//
        viewMail(item) {
            this.editedIndex = this.controlesOP.indexOf(item);
            this.editedItem = Object.assign({}, item);
            this.formId = this.editedItem.idForm;
            this.dialogMail = true;
        },
        closeMail() {
            this.dialogMail = false;
            this.$nextTick(() => {
                this.editedItem = Object.assign({}, this.defaultItem);
                this.editedIndex = -1;
            });
        },
        async sendMail() {
            if (!this.$refs.dialogForm.validate()) return;
            this.loading = true;
            let Columns = [{
                    header: "Titre du contrôle",
                    key: "titleControleOp",
                }, {
                    header: "LVP 1",
                    key: "lvp1",
                }, {
                    header: "LVP 2",
                    key: "lvp2",
                }, {
                    header: "LVP 3",
                    key: "lvp3",
                }, {
                    header: "Statut",
                    key: "status"
                },
                {
                    header: "Agent",
                    key: "userName",
                },

                {
                    header: "Adresse email de l'agent",
                    key: "userEmail"
                },
                {
                    header: "Date et heure d'envoi",
                    key: "dateEnvoi"
                },
                {
                    header: "Feux et balises de piste allumés",
                    key: "feuBalisesAllume"
                },
                {
                    header: "Information transmise aux différents intervenants",
                    key: "informationTransmises",
                },
                {
                    header: "Observation 1",
                    key: "firstSectionObs"
                },
                {
                    header: "Feux et balises de piste allumés",
                    key: "feuBalisesAllume2",
                },
                {
                    header: "Information transmise aux différents intervenants",
                    key: "informationTransmises2",
                },
                {
                    header: "Dispositifs d’éclairage extérieurs",
                    key: "dispositifEclairageExterieurZoneReserveEnMarche",
                },
                {
                    header: "Respect des vitesses de circulation",
                    key: "respectVitesse",
                },
                {
                    header: "Dispositif antibrouillard disponible et allumé sur les engins",
                    key: "dispositifAntiBrouillardAllume",
                },
                {
                    header: "Zone de travaux et  zone fermée sont délimitées",
                    key: "zoneTravauxZoneFermeDelimite",
                },
                {
                    header: "Balisage lumineux de l’aire de trafic",
                    key: "balisageLumineuxAllume",
                },
                {
                    header: "Travaux d’entretien sur l’aire de trafic sont suspendus",
                    key: "TravauxEntretienSuspenduEtPersonnelDegage",
                },
                {
                    header: "Accès à l’aire de manœuvre limité",
                    key: "accesAireDeManeouvreLimite",
                },
                {
                    header: "Services de contrôle aérien avertis",
                    key: "serviceControlAvertiDeToutePanne",
                },
                {
                    header: "Observation 2",
                    key: "seocondSectionObs"
                },
                {
                    header: "Formulaire de compte rendu d’évènement renseigné",
                    key: "formulaireCompteRenduEvenementRensiegne",
                },
                {
                    header: "Observation 3",
                    key: "thirdSectionObs"
                },
                {
                    header: "UPRA informé par la TWR",
                    key: "urpaInformeParTwr",
                },
                {
                    header: "Vérifications visuelles par UPRA",
                    key: "verificationvisuelParUrpa",
                },
                {
                    header: "UPRA informe le commandant de permanence de la SALT",
                    key: "urpaInformeCommandant",
                },
                {
                    header: "UPRA informe les autres intervenants",
                    key: "urpaInformeAutreIntervenant",
                },
                {
                    header: "Observation 4",
                    key: "fourthSectionObs",
                },
            ];
            this.loading = true;
            var m = new Date();
            var dateString =
                ("0" + m.getUTCDate()).slice(-2) + "-" +
                ("0" + (m.getUTCMonth() + 1)).slice(-2) + "-" +
                m.getUTCFullYear() + " " +
                ("0" + m.getUTCHours()).slice(-2) + ":" +
                ("0" + m.getUTCMinutes()).slice(-2);
            const snapshot = await firestore().collection("controleOp").where("idForm", "==", this.formId).get();
            const documents = snapshot.docs.map((doc) => doc.data());
            const doc = new jsPDF();
            let pageHeight = doc.internal.pageSize.height;
            doc.addImage("data:image/png;base64,", 'JPEG', 20, 0, 25, 25)
            doc.setFontSize(16);
            doc.setFont("helvetica");
            doc.setTextColor(255, 0, 0);
            doc.text("Contrôle des opérations (Code: " + this.formId + ")", 105, 15, null, null, "center");
            doc.setTextColor(0, 2, 3);
            let i = 35;
            Columns.forEach((element) => {
                doc.text(element["header"] + " : " + documents[0][element["key"]], 10, i, {
                    align: "left",
                });
                if (i >= pageHeight) {
                    doc.addPage();
                    i = 15
                } else {
                    i = i + 10;
                }
            });
            doc.setTextColor(51, 10, 199);
            doc.text("Ce document a été généré par le système GES-UPRA au " + dateString, 105, pageHeight - 5, {
                align: "center",
            });
            var pdfBase64 = doc.output('datauristring');

            let config = {
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": "key=AAAALeem-1k:APA91bFi6XX1XOe2gz-VGRBEKI00LfWbXS8TpYRQKzKzc9L3mVKJgZwXWJuNZetn5F316V1ePT_qwFyrR-6q7nwVGs7Hp649JGlEb-wUdpRfDyfvrEcaUxFlsGEjjMXjZY6ap2pQwY-Q"
                }
            };
            let body = {
                pdf: pdfBase64,
                formId: this.formId,
                email: this.email,
            };
            await AXIOS.post("https://thawing-badlands-05581.herokuapp.com/sendMail", body, config);
            this.loading = false;
            this.snack = true;
            this.snackColor = "success";
            this.snackText = "Mail envoyé avec succès.";
            this.dialogMail = false;
        },
        //send mail to user finish//

        async deleteItem(item) {
            this.loading = true;
            if (confirm("Etes vous sûr(e) de vouloir supprimer ce formulaire?")) {
                this.loading = true;
                try {
                    await this.removecontrolesOP(item);
                    this.loading = false;
                    this.snack = true;
                    this.snackColor = "success";
                    this.snackText = "Formulaire supprimé avec succès.";
                } catch (e) {
                    this.loading = false;
                    this.snack = true;
                    this.snackColor = "error";
                    this.snackText = "Formulaire non supprimé";
                    console.error(e);
                }
            } else {
                this.loading = false;
            }
        },
        async exportFormToPDF(idForm) {
            let Columns = [{
                    header: "Titre du contrôle",
                    key: "titleControleOp",
                }, {
                    header: "LVP 1",
                    key: "lvp1",
                }, {
                    header: "LVP 2",
                    key: "lvp2",
                }, {
                    header: "LVP 3",
                    key: "lvp3",
                }, {
                    header: "Statut",
                    key: "status"
                },
                {
                    header: "Agent",
                    key: "userName",
                },

                {
                    header: "Adresse email de l'agent",
                    key: "userEmail"
                },
                {
                    header: "Date et heure d'envoi",
                    key: "dateEnvoi"
                },
                {
                    header: "Feux et balises de piste allumés",
                    key: "feuBalisesAllume"
                },
                {
                    header: "Information transmise aux différents intervenants",
                    key: "informationTransmises",
                },
                {
                    header: "Observation 1",
                    key: "firstSectionObs"
                },
                {
                    header: "Feux et balises de piste allumés",
                    key: "feuBalisesAllume2",
                },
                {
                    header: "Information transmise aux différents intervenants",
                    key: "informationTransmises2",
                },
                {
                    header: "Dispositifs d’éclairage extérieurs",
                    key: "dispositifEclairageExterieurZoneReserveEnMarche",
                },
                {
                    header: "Respect des vitesses de circulation",
                    key: "respectVitesse",
                },
                {
                    header: "Dispositif antibrouillard disponible et allumé sur les engins",
                    key: "dispositifAntiBrouillardAllume",
                },
                {
                    header: "Zone de travaux et  zone fermée sont délimitées",
                    key: "zoneTravauxZoneFermeDelimite",
                },
                {
                    header: "Balisage lumineux de l’aire de trafic",
                    key: "balisageLumineuxAllume",
                },
                {
                    header: "Travaux d’entretien sur l’aire de trafic sont suspendus",
                    key: "TravauxEntretienSuspenduEtPersonnelDegage",
                },
                {
                    header: "Accès à l’aire de manœuvre limité",
                    key: "accesAireDeManeouvreLimite",
                },
                {
                    header: "Services de contrôle aérien avertis",
                    key: "serviceControlAvertiDeToutePanne",
                },
                {
                    header: "Observation 2",
                    key: "seocondSectionObs"
                },
                {
                    header: "Formulaire de compte rendu d’évènement renseigné",
                    key: "formulaireCompteRenduEvenementRensiegne",
                },
                {
                    header: "Observation 3",
                    key: "thirdSectionObs"
                },
                {
                    header: "UPRA informé par la TWR",
                    key: "urpaInformeParTwr",
                },
                {
                    header: "Vérifications visuelles par UPRA",
                    key: "verificationvisuelParUrpa",
                },
                {
                    header: "UPRA informe le commandant de permanence de la SALT",
                    key: "urpaInformeCommandant",
                },
                {
                    header: "UPRA informe les autres intervenants",
                    key: "urpaInformeAutreIntervenant",
                },
                {
                    header: "Observation 4",
                    key: "fourthSectionObs",
                },

                // { header: "Commentaires de l'administrateur", key: "commentaireAdmin" },

            ];
            this.loading = true;
            var m = new Date();
            var dateString =
                ("0" + m.getUTCDate()).slice(-2) + "-" +
                ("0" + (m.getUTCMonth() + 1)).slice(-2) + "-" +
                m.getUTCFullYear() + " " +
                ("0" + m.getUTCHours()).slice(-2) + ":" +
                ("0" + m.getUTCMinutes()).slice(-2);
            const snapshot = await firestore().collection("controleOp").where("idForm", "==", idForm).get();
            const documents = snapshot.docs.map((doc) => doc.data());
            const doc = new jsPDF();
            let pageHeight = doc.internal.pageSize.height;
            doc.addImage("data:image/png;base64,", 'JPEG', 20, 0, 25, 25)
            doc.setFontSize(16);
            doc.setFont("helvetica");
            doc.setTextColor(255, 0, 0);
            doc.text("Contrôle des opérations (Code: " + idForm + ")", 105, 15, null, null, "center");
            doc.setTextColor(0, 2, 3);
            let i = 35;
            Columns.forEach((element) => {
                doc.text(element["header"] + " : " + documents[0][element["key"]], 10, i, {
                    align: "left",
                });
                if (i >= pageHeight) {
                    doc.addPage();
                    i = 15
                } else {
                    i = i + 10;
                }
            });
            doc.setTextColor(51, 10, 199);
            doc.text("Ce document a été généré par le système GES-UPRA au " + dateString, 105, pageHeight - 5, {
                align: "center",
            });
            doc.save("UPRA FORM (" + idForm + ").pdf");
            this.loading = false;
        },

        async printForm(idForm) {
            let Columns = [{
                    header: "Titre du contrôle",
                    key: "titleControleOp",
                }, {
                    header: "LVP 1",
                    key: "lvp1",
                }, {
                    header: "LVP 2",
                    key: "lvp2",
                }, {
                    header: "LVP 3",
                    key: "lvp3",
                }, {
                    header: "Statut",
                    key: "status"
                },
                {
                    header: "Agent",
                    key: "userName",
                },

                {
                    header: "Adresse email de l'agent",
                    key: "userEmail"
                },
                {
                    header: "Date et heure d'envoi",
                    key: "dateEnvoi"
                },
                {
                    header: "Feux et balises de piste allumés",
                    key: "feuBalisesAllume"
                },
                {
                    header: "Information transmise aux différents intervenants",
                    key: "informationTransmises",
                },
                {
                    header: "Observation 1",
                    key: "firstSectionObs"
                },
                {
                    header: "Feux et balises de piste allumés",
                    key: "feuBalisesAllume2",
                },
                {
                    header: "Information transmise aux différents intervenants",
                    key: "informationTransmises2",
                },
                {
                    header: "Dispositifs d’éclairage extérieurs",
                    key: "dispositifEclairageExterieurZoneReserveEnMarche",
                },
                {
                    header: "Respect des vitesses de circulation",
                    key: "respectVitesse",
                },
                {
                    header: "Dispositif antibrouillard disponible et allumé sur les engins",
                    key: "dispositifAntiBrouillardAllume",
                },
                {
                    header: "Zone de travaux et  zone fermée sont délimitées",
                    key: "zoneTravauxZoneFermeDelimite",
                },
                {
                    header: "Balisage lumineux de l’aire de trafic",
                    key: "balisageLumineuxAllume",
                },
                {
                    header: "Travaux d’entretien sur l’aire de trafic sont suspendus",
                    key: "TravauxEntretienSuspenduEtPersonnelDegage",
                },
                {
                    header: "Accès à l’aire de manœuvre limité",
                    key: "accesAireDeManeouvreLimite",
                },
                {
                    header: "Services de contrôle aérien avertis",
                    key: "serviceControlAvertiDeToutePanne",
                },
                {
                    header: "Observation 2",
                    key: "seocondSectionObs"
                },
                {
                    header: "Formulaire de compte rendu d’évènement renseigné",
                    key: "formulaireCompteRenduEvenementRensiegne",
                },
                {
                    header: "Observation 3",
                    key: "thirdSectionObs"
                },
                {
                    header: "UPRA informé par la TWR",
                    key: "urpaInformeParTwr",
                },
                {
                    header: "Vérifications visuelles par UPRA",
                    key: "verificationvisuelParUrpa",
                },
                {
                    header: "UPRA informe le commandant de permanence de la SALT",
                    key: "urpaInformeCommandant",
                },
                {
                    header: "UPRA informe les autres intervenants",
                    key: "urpaInformeAutreIntervenant",
                },
                {
                    header: "Observation 4",
                    key: "fourthSectionObs",
                },

                // { header: "Commentaires de l'administrateur", key: "commentaireAdmin" },

            ];
            this.loading = true;
            var m = new Date();
            var dateString =
                ("0" + m.getUTCDate()).slice(-2) + "-" +
                ("0" + (m.getUTCMonth() + 1)).slice(-2) + "-" +
                m.getUTCFullYear() + " " +
                ("0" + m.getUTCHours()).slice(-2) + ":" +
                ("0" + m.getUTCMinutes()).slice(-2);
            const snapshot = await firestore().collection("controleOp").where("idForm", "==", idForm).get();
            const documents = snapshot.docs.map((doc) => doc.data());
            const doc = new jsPDF();
            let pageHeight = doc.internal.pageSize.height;

            doc.addImage("data:image/png;base64,", 'JPEG', 20, 0, 25, 25)
            doc.setFontSize(16);
            doc.setFont("helvetica");
            doc.setTextColor(255, 0, 0);
            doc.text("Contrôle des opérations (Code: " + idForm + ")", 105, 15, null, null, "center");
            doc.setTextColor(0, 2, 3);
            let i = 35;
            Columns.forEach((element) => {
                doc.text(element["header"] + " : " + documents[0][element["key"]], 10, i, {
                    align: "left",
                });
                if (i >= pageHeight) {
                    doc.addPage();
                    i = 15
                } else {
                    i = i + 10;
                }
            });
            doc.setTextColor(51, 10, 199);
            doc.text("Ce document a été généré par le système GES-UPRA au " + dateString, 105, pageHeight - 5, {
                align: "center",
            });
            doc.autoPrint({
                variant: 'non-conform'
            });
            doc.output('dataurlnewwindow');
            this.loading = false;
        },

        addItem() {
            this.editedIndex = -1;
            this.defaultItem = Object.assign({}, this.defaultItemInit);
            this.dialogAdd = true;
        },
        closeAdd() {
            this.dialogAdd = false;
            this.$nextTick(() => {
                this.editedItem = Object.assign({}, this.defaultItem);
                this.editedIndex = -1;
            });
        },
        ...mapActions({
            loadcontrolesOP: "controlesOP/loadcontrolesOP",
        }),
        openLink(link) {
            window.open(link, "_blank");
        },
        ...mapActions({
            addcontroleOP: "controlesOP/addcontrolesOP",
            updatecontrolesOP: "controlesOP/updatecontrolesOP",
            removecontrolesOP: "controlesOP/removecontrolesOP",
        }),
        exportPDF() {
            this.loading2 = true;
            let exportColumns = [{
                    title: "ID",
                    dataKey: "idForm",
                },
                {
                    title: "Titre",
                    dataKey: "titleControleOp",
                }, {
                    title: "LVP 1",
                    dataKey: "lvp1",
                }, {
                    title: "LVP 2",
                    dataKey: "lvp2",
                }, {
                    title: "LVP 3",
                    dataKey: "lvp3",
                },
                {
                    title: "Agent",
                    dataKey: "userName",
                },

                {
                    title: "Adresse email",
                    dataKey: "userEmail"
                },
                {
                    title: "Date et heure d'envoi",
                    dataKey: "dateEnvoi"
                },

                {
                    title: "UPRA informé par la TWR",
                    dataKey: "urpaInformeParTwr",
                },
                {
                    title: "Vérifications visuelles par UPRA",
                    dataKey: "verificationvisuelParUrpa",
                },
                {
                    title: "UPRA informe le commandant de permanence de la SALT",
                    dataKey: "urpaInformeCommandant",
                },
                {
                    title: "UPRA informe les autres intervenants sur l’aire de trafic')",
                    dataKey: "urpaInformeAutreIntervenant",
                },

                // { title: "Commentaires de l'administrateur", dataKey: "commentaireAdmin" },
                {
                    title: "Statut",
                    dataKey: "status"
                },
            ];
            import("jspdf").then(jsPDF => {
                import("jspdf-autotable").then(() => {
                    const doc = new jsPDF.default('l', 'pt', "a4");
                    doc.setFontSize(16);
                    doc.setFont("helvetica");
                    doc.text("Liste de contrôles des opérations LVP", 30, 30, {
                        align: "justify",
                    });
                    doc.autoTable(exportColumns, this.controlesOP);
                    doc.save("Contrôle des opérations LVP.pdf");
                    this.loading2 = false;
                })
            });
        },
        //To upload
        async uploadTaskImages() {
            this.snack = true;
            this.snackColor = "danger";
            this.snackText = this.$refs.uploader.files.length + " images prises en compte";
            let vm = this;
            vm.defaultItem.imagesJoined = [];
            for (let i = 0; i < this.$refs.uploader.files.length; i++) {
                const newImage = this.$refs.uploader.files[i];
                if (newImage != null && newImage.name) {
                    let ref = storage()
                        .ref()
                        .child("images/" + newImage.name);
                    await ref.put(newImage).then((snapshot) => {
                        if (snapshot.state === "success") {
                            snapshot.ref.getDownloadURL().then(function (downloadURL) {
                                vm.defaultItem.imagesJoined.push(downloadURL);
                            });
                        }
                    });
                }
            }
        },

        async uploadTaskDocuments() {
            this.snack = true;
            this.snackColor = "danger";
            this.snackText = this.$refs.uploader1.files.length + " documents prises en compte";
            let vm = this;
            vm.defaultItem.documentsJoined = [];
            for (let i = 0; i < this.$refs.uploader1.files.length; i++) {
                const newDocument = this.$refs.uploader1.files[i];
                if (newDocument != null && newDocument.name) {
                    let ref = storage()
                        .ref()
                        .child("documents/" + newDocument.name);
                    await ref.put(newDocument).then((snapshot) => {
                        if (snapshot.state === "success") {
                            snapshot.ref.getDownloadURL().then(function (downloadURL) {
                                vm.defaultItem.documentsJoined.push(downloadURL);
                            });
                        }
                    });
                }
            }
        },
        //-------------------------------------------------------------end upload

        getcontrolesOPById: function (id) {
            let controlesOP = this.controlesOP.filter((c) => c.id == id)[0];
            console.log(controlesOP.status);
            if (controlesOP === undefined) {
                console.log("undefined qarşim");
                return {
                    nom: "Unknown"
                };
            }
            return controlesOP;
        },
        async exportToExcel() {
            this.loading1 = true;
            let workbook = new Excel.Workbook();
            let worksheet = workbook.addWorksheet("Contrôle des opérations LVP");
            worksheet.columns = [{
                    header: "ID",
                    key: "idForm",
                },
                {
                    header: "Titre du contrôle",
                    key: "titleControleOp",
                }, {
                    header: "Type LVP",
                    key: "typeLVP",
                },
                {
                    header: "Agent",
                    key: "userName",
                },

                {
                    header: "Adresse email de l'agent",
                    key: "userEmail"
                },
                {
                    header: "Date et heure d'envoi",
                    key: "dateEnvoi"
                },
                {
                    header: "Feux et balises de piste allumés",
                    key: "feuBalisesAllume"
                },
                {
                    header: "Information transmise aux différents intervenants",
                    key: "informationTransmises",
                },
                {
                    header: "Observation 1",
                    key: "firstSectionObs"
                },
                {
                    header: "Feux et balises de piste allumés",
                    key: "feuBalisesAllume2",
                },
                {
                    header: "Information transmise aux différents intervenants",
                    key: "informationTransmises2",
                },
                {
                    header: "Dispositifs d’éclairage extérieurs des zones réservées sont mis en marche",
                    key: "dispositifEclairageExterieurZoneReserveEnMarche",
                },
                {
                    header: "Respect des vitesses de circulation (10km/h dans les couloirs, 5km/h par heure autour des aéronefs')",
                    key: "respectVitesse",
                },
                {
                    header: "Dispositif antibrouillard disponible et allumé sur les engins",
                    key: "dispositifAntiBrouillardAllume",
                },
                {
                    header: "Zone de travaux et  zone fermée sont délimitées",
                    key: "zoneTravauxZoneFermeDelimite",
                },
                {
                    header: "Balisage lumineux de l’aire de trafic",
                    key: "balisageLumineuxAllume",
                },
                {
                    header: "Travaux d’entretien sur l’aire de trafic sont suspendus",
                    key: "TravauxEntretienSuspenduEtPersonnelDegage",
                },
                {
                    header: "Accès à l’aire de manœuvre limité uniquement aux véhicules de SLI, PRA et de maintenance ASECNA",
                    key: "accesAireDeManeouvreLimite",
                },
                {
                    header: "Services de contrôle aérien avertis de toute panne ou défaillance des aides visuelles",
                    key: "serviceControlAvertiDeToutePanne",
                },
                {
                    header: "Observation 2",
                    key: "seocondSectionObs"
                },
                {
                    header: "Formulaire de compte rendu d’évènement renseigné",
                    key: "formulaireCompteRenduEvenementRensiegne",
                },
                {
                    header: "Observation 3",
                    key: "thirdSectionObs"
                },
                {
                    header: "UPRA informé par la TWR",
                    key: "urpaInformeParTwr",
                },
                {
                    header: "Vérifications visuelles par UPRA",
                    key: "verificationvisuelParUrpa",
                },
                {
                    header: "UPRA informe le commandant de permanence de la SALT",
                    key: "urpaInformeCommandant",
                },
                {
                    header: "UPRA informe les autres intervenants sur l’aire de trafic')",
                    key: "urpaInformeAutreIntervenant",
                },
                {
                    header: "Observation 4",
                    key: "fourthSectionObs",
                },

                // { header: "Commentaires de l'administrateur", key: "commentaireAdmin" },
                {
                    header: "Statut",
                    key: "status"
                },
            ];
            worksheet.columns.forEach(function (column) {
                var maxLength = 0;
                column["eachCell"]({
                    includeEmpty: true
                }, function (cell) {
                    var columnLength = cell.value ? cell.value.toString().length : 10;
                    if (columnLength > maxLength) {
                        maxLength = columnLength;
                    }
                });
                column.width = maxLength < 10 ? 10 : maxLength;
            });
            worksheet.getRow(1).font = {
                bold: true,
                color: {
                    argb: "0047AB"
                }
            };
            const snapshot = await firestore().collection("controleOp").get();
            snapshot.forEach((doc) => {
                let e = doc.data();
                worksheet.addRow({
                    ...e,
                });
            });
            workbook.xlsx.writeBuffer().then((data) => {
                const blob = new Blob([data], {
                    type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                });
                fs.saveAs(blob, `controlesOp.xlsx`);
            });
            this.loading1 = false;
        },
        async initialize() {
            this.loading = true;
            try {
                await this.loadcontrolesOP();
            } catch (e) {
                console.error(e);
            }
            this.loading = false;
        },

        editItem(item) {
            this.editedIndex = this.controlesOP.indexOf(item);
            this.editedItem = Object.assign({}, item);
            this.dialog = true;
        },

        viewItem(item) {
            this.editedIndex = this.controlesOP.indexOf(item);
            this.editedItem = Object.assign({}, item);
            this.dialogView = true;
        },

        close() {
            this.dialog = false;
            this.$nextTick(() => {
                this.editedItem = Object.assign({}, this.defaultItem);
                this.editedIndex = -1;
            });
        },

        closeView() {
            this.dialogView = false;
            this.$nextTick(() => {
                this.editedItem = Object.assign({}, this.defaultItem);
                this.editedIndex = -1;
            });
        },

        async save() {
            if (!this.$refs.dialogForm.validate()) return;
            this.loading = true;
            if (this.editedIndex <= -1) {
                this.snack = true;
                this.snackColor = "blue";
                this.snackText = "Action en cours...";
                await this.uploadTaskImages();
                await this.uploadTaskDocuments();
            }

            if (this.editedIndex > -1) {
                this.loading = true;
                try {
                    await this.updatecontrolesOP({
                        index: this.editedIndex,
                        order: this.editedItem,
                    });
                    this.loading = false;
                    this.dialogView = false;
                    this.dialogAdd = false;

                    this.close();
                    this.snack = true;
                    this.snackColor = "success";
                    this.snackText = "Le statut du formulaire a été mise à jour";
                } catch (e) {
                    this.loading = false;
                    this.close();

                    this.snack = true;
                    this.snackColor = "error";
                    this.snackText =
                        "Le statut du formulaire n'a pas pu être mise à jour";
                    console.error(e);
                }
            } else {
                this.loading = true;
                try {
                    await this.addcontroleOP(this.defaultItem);
                    this.loading = false;
                    this.close();
                    this.dialogAdd = false;
                    this.snack = true
                    this.snackColor = 'success'
                    this.snackText = 'Formulaire ajouté avec succès.'
                } catch (e) {
                    this.loading = false;
                    this.close();
                    this.snack = true
                    this.snackColor = 'error'
                    this.snackText = 'Le formulaire n\'a pas été ajouté.'
                    console.error(e);
                }
            }
        },
    },
    filters: {
        truncate: function (text, length, suffix) {
            if (text.length > length) {
                return text.substring(0, length) + suffix;
            } else {
                return text;
            }
        },
    },
};
</script>
