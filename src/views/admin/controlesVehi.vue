<template>
<v-card class="spacing-playground ma-5">
    <v-data-table :options="{ openLink }" :headers="headers" :items="controlesVehi" :search="search" class="elevation-4" :loading="loading" loading-text="Chargement...Veuillez patienter">
        <template v-slot:top>
            <v-toolbar flat>
                <v-toolbar-title class="underline">Liste</v-toolbar-title>
                <v-spacer />

                <v-dialog v-model="dialog" max-width="700px">
                    <base-material-card>
                        <template v-slot:heading>
                            <div class="display-2 font-weight-light">
                                {{ formTitle }}
                            </div>

                            <div class="subtitle-1 font-weight-light">
                                Completer les informations
                            </div>
                        </template>
                        <v-form v-bind:disabled="loading" lazy-validation ref="dialogForm">
                            <v-row>
                                <v-col cols="12">
                                    <v-select prepend-inner-icon="mdi-apps" label="Status" :disabled="loading" v-model="editedItem.status" :items="items2"></v-select>
                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12">
                                    <h4>Votre commentaire</h4>
                                    <vue-editor v-model="editedItem.commentaireAdmin" label="Commentaires de l'admin"></vue-editor>

                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12" class="text-right">
                                    <v-btn :disabled="loading" class="success" color="" text @click="save">Enregistrer</v-btn>
                                    <v-btn :disabled="loading" color="" text @click="close">Quitter</v-btn>
                                </v-col>
                            </v-row>
                        </v-form>
                    </base-material-card>
                </v-dialog>

                <v-dialog v-model="dialogAdd" max-width="700px">
                    <base-material-card>
                        <template v-slot:heading>
                            <div class="display-2 font-weight-light">
                                Ajouter un contrôle de véhicule
                            </div>

                            <div class="subtitle-1 font-weight-light">
                                Completer les informations
                            </div>
                        </template>
                        <v-form v-bind:disabled="loading" lazy-validation ref="dialogForm">
                            <v-row>
                                <v-col cols="12">

                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Intitulé</h4>
                                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline" v-model="defaultItem.titleControleVehi" label="Titre du contrôle" :rules="titleRule">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <!-- <center>
                          <h2 style="font-weight: bold">Véhicule 1</h2>
                        </center> -->
                                    <!-- <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline"
                              v-model="defaultItem.titleVehicule1" label="Nom du véhicule">
                            </v-text-field>
                          </v-col>
                        </v-row> -->
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline" v-model="defaultItem.immatriculation" label="Immatriculation du véhicule">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.numeroEngin" label="Numéro de l'engin">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.societe" label="Nom de la société"></v-text-field>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.Qimmatriculation1" label="Immatriculation ?">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.macaron" label="Macaron"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.permisCivile" label="Permis civile">
                                            </v-select>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.permisAtAm" label="Permis « AT » / « AM »">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.carteGrise" label="Carte grise"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.planQuadrille" label="Plan quadrillé">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.logo" label="Logo"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.gyrophare" label="Gyrophare"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.radio" label="Équipement radio"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.extincteur" label="Extincteur"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.boitePharmacie" label="Boite à pharmacie">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.triangleSecu" label="Triangles de sécurité /cônes fluorescentes">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Observations</h4>
                                            <vue-editor v-model="editedItem.observationVehicule1"></vue-editor>

                                        </v-col>
                                    </v-row>
                                    <v-divider height="20px"></v-divider>
                                    <!-- <br />
                        <center>
                          <h2 style="font-weight: bold">Véhicule 2</h2>
                        </center>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline"
                              v-model="defaultItem.titleVehicule2" label="Nom du véhicule">
                            </v-text-field>
                          </v-col>
                        </v-row>

                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline"
                              v-model="defaultItem.immatriculation2" label="Immatriculation du véhicule"></v-text-field>
                          </v-col>
                        </v-row>

                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-text-field prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.numeroEngin2" label="Numéro de l'engin">
                            </v-text-field>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.societe2"
                              label="Nom de la société">
                            </v-text-field>
                          </v-col>
                        </v-row>

                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.Qimmatriculation2" label="Immatriculation ? ">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.macaron2" label="Macaron"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.permisCivile2" label="Permis civile">
                            </v-select>
                          </v-col>
                        </v-row>

                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.permisAtAm2" label="Permis « AT » / « AM »">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.carteGrise2" label="Carte grise"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.planQuadrille2" label="Plan quadrillé">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.logo2" label="Logo"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.gyrophare2" label="Gyrophare"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.radio2" label="Équipement radio"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.extincteur2" label="Extincteur"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.boitePharmacie2" label="Boite à pharmacie">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="defaultItem.triangleSecu2" label="Triangles de sécurité /cônes fluorescentes">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12" sm="10" md="10">-
                            <h4>Observations sur le véhicule 2</h4>
                             <vue-editor v-model="editedItem.observationVehicule2"></vue-editor>

                          </v-col>
                        </v-row> -->
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items2" prepend-inner-icon="mdi-format-align-justify" v-model="defaultItem.status" label="Statut"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            Détails du statut
                                            <vue-editor v-model="defaultItem.detailStatut" label="Détails du statut"> </vue-editor>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Votre commentaire</h4>
                                            <!-- <v-textarea prepend-inner-icon="mdi-format-align-justify" 
                               v-model="defaultItem.commentaireAdmin" label="Commentaires de l'admin">
                            </v-textarea> -->
                                            <vue-editor v-model="editedItem.commentaireAdmin" label="Commentaires de l'admin">
                                            </vue-editor>

                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <input type="file" multiple class="d-none" ref="uploader" accept="image/*" @change="uploadTaskImages()" />
                                            <v-btn :disabled="loading" color="primary" v-model="imagePret" @click="$refs.uploader.click()">
                                                <v-icon dark>
                                                    mdi-image-multiple
                                                </v-icon> <b>Ajouter des images</b>
                                            </v-btn>
                                        </v-col>

                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <input type="file" multiple class="d-none" ref="uploader1" accept=".xlsx,.xls,.doc, .docx,.ppt, .pptx,.txt,.pdf" @change="uploadTaskDocuments()" />
                                            <v-btn :disabled="loading" color="red" v-model="documentPret" @click="$refs.uploader1.click()">
                                                <v-icon dark>
                                                    mdi-file-arrow-up-down
                                                </v-icon> <b>Ajouter des documents</b>
                                            </v-btn>

                                        </v-col>

                                    </v-row>
                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12" class="text-right">
                                    <v-btn :disabled="loading" class="success" color="" text @click="save">Enregistrer</v-btn>
                                    <v-btn :disabled="loading" color="" text @click="closeAdd">Quitter</v-btn>
                                </v-col>
                            </v-row>
                        </v-form>
                    </base-material-card>
                </v-dialog>

                <v-dialog v-model="dialogView" max-width="700px">
                    <base-material-card>
                        <template v-slot:heading>
                            <div class="display-2 font-weight-light">
                                Détails d'un contrôle de véhicule
                            </div>

                            <div class="subtitle-1 font-weight-light">
                                Completer les informations
                            </div>
                        </template>
                        <v-form v-bind:disabled="loading" lazy-validation ref="dialogForm">
                            <v-row>
                                <v-col cols="12">
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline" v-model="editedItem.userName" label="Nom et prénoms" disabled="true"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <!-- <center>
                          <h2 style="font-weight: bold">Véhicule 1</h2>
                        </center> -->
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Intitulé</h4>
                                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline" v-model="editedItem.titleControleVehi" label="Titre du contrôle" :rules="titleRule">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <!-- <center>
                          <h2 style="font-weight: bold">Véhicule 1</h2>
                        </center> -->
                                    <!-- <v-row>
                          <v-col cols="12">
                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline"
                              v-model="editedItem.titleVehicule1" label="Nom du véhicule">
                            </v-text-field>
                          </v-col>
                        </v-row> -->
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline" v-model="editedItem.immatriculation" label="Immatriculation du véhicule"></v-text-field>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.numeroEngin" label="Numéro de l'engin">
                                            </v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.societe" label="Nom de la société"></v-text-field>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.Qimmatriculation1" label="Immatriculation ?">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.macaron" label="Macaron"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.permisCivile" label="Permis civile">
                                            </v-select>
                                        </v-col>
                                    </v-row>

                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.permisAtAm" label="Permis « AT » / « AM »">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.carteGrise" label="Carte grise"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.planQuadrille" label="Plan quadrillé">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.logo" label="Logo"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.gyrophare" label="Gyrophare"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.radio" label="Équipement radio"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.extincteur" label="Extincteur"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.boitePharmacie" label="Boite à pharmacie">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.triangleSecu" label="Triangles de sécurité /cônes fluorescentes">
                                            </v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">-
                                            <h4>Observations sur le véhicule 1</h4>
                                            <vue-editor v-model="editedItem.observationVehicule1"></vue-editor>

                                        </v-col>
                                    </v-row>
                                    <v-divider height="20px"></v-divider>
                                    <!-- <br />
                        <center>
                          <h2 style="font-weight: bold">Véhicule 2</h2>
                        </center>
                        <v-row>
                          <v-col cols="12">
                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline"
                              v-model="editedItem.titleVehicule2" label="Nom du véhicule">
                            </v-text-field>
                          </v-col>
                        </v-row>

                        <v-row>
                          <v-col cols="12">
                            <v-text-field outlined prepend-inner-icon="mdi-account-details-outline"
                              v-model="editedItem.immatriculation2" label="Immatriculation du véhicule"></v-text-field>
                          </v-col>
                        </v-row>

                        <v-row>
                          <v-col cols="12">
                            <v-text-field prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.numeroEngin2" label="Numéro de l'engin">
                            </v-text-field>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-text-field prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.societe2"
                              label="Nom de la société">
                            </v-text-field>
                          </v-col>
                        </v-row>

                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.Qimmatriculation2" label="Immatriculation ? ">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.macaron2" label="Macaron"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.permisCivile2" label="Permis civile">
                            </v-select>
                          </v-col>
                        </v-row>

                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.permisAtAm2" label="Permis « AT » / « AM »">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.carteGrise2" label="Carte grise"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.planQuadrille2" label="Plan quadrillé">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.logo2" label="Logo"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.gyrophare2" label="Gyrophare"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.radio2" label="Équipement radio"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.extincteur2" label="Extincteur"></v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.boitePharmacie2" label="Boite à pharmacie">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">
                            <v-select :items="items3" prepend-inner-icon="mdi-format-align-justify"
                              v-model="editedItem.triangleSecu2" label="Triangles de sécurité /cônes fluorescentes">
                            </v-select>
                          </v-col>
                        </v-row>
                        <v-row>
                          <v-col cols="12">-
                            <h4>Observations sur le véhicule 2</h4>
                             <vue-editor v-model="editedItem.observationVehicule2"></vue-editor>

                          </v-col>
                        </v-row> -->
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select :items="items2" prepend-inner-icon="mdi-format-align-justify" v-model="editedItem.status" label="Statut"></v-select>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            Détails du statut
                                            <vue-editor v-model="editedItem.detailStatut" label="Détails du statut"> </vue-editor>
                                        </v-col>
                                    </v-row>
                                    <v-divider></v-divider>
                                    <v-spacer></v-spacer>
                                    <v-col cols="12" v-if="editedItem.customFieldsData && editedItem.customFieldsData.length > 0">
                                        <h2 style="color: red; text-align: center;">Champs personnalisés</h2>
                                        <div v-for="item of editedItem.customFieldsData" v-bind:key="item.cle" v-bind:video="item">

                                            <v-row v-if="item.type == 'Zone de texte'">
                                                <v-col cols="12">
                                                    <h4>{{ item.titre }}</h4>
                                                    <vue-editor v-model="editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur">
                                                    </vue-editor>

                                                </v-col>
                                            </v-row>

                                            <v-row v-if="item.type == 'Texte' || item.type == 'Numérique'">
                                                <v-col cols="12">
                                                    <h4>{{ item.titre }}</h4>
                                                    <v-text-field prepend-inner-icon="mdi-format-align-justify" :disabled="loading" v-model="editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur">
                                                    </v-text-field>
                                                </v-col>
                                            </v-row>

                                            <v-row v-if="item.type == 'Date'">
                                                <v-col cols="12">
                                                    <h4>{{ item.titre }}</h4>

                                                    <v-text-field v-model="editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur" readonly v-bind="attrs" v-on="on"></v-text-field>

                                                </v-col>
                                            </v-row>

                                            <v-row v-if="item.type == 'Case à cocher'">
                                                <v-col cols="12">
                                                    <v-checkbox v-model="editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur" :label="item.titre"></v-checkbox>
                                                </v-col>
                                            </v-row>

                                            <v-row v-if="item.type == 'Liste déroulante'">
                                                <v-col cols="12">
                                                    <v-select prepend-inner-icon="mdi-apps" :label="item.titre" :disabled="loading" v-model="editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur" :items="item.listData"></v-select>
                                                </v-col>
                                            </v-row>

                                            <v-row v-if="item.type == 'Fichier'">
                                                <v-col cols="12">
                                                    <h4>{{ item.titre }}</h4>
                                                    <v-btn class="ma-2" color="primary" @click="() => openLink(editedItem.customFieldsData[editedItem.customFieldsData.indexOf(item)].valeur)" download>
                                                        VOIR LE FICHIER
                                                    </v-btn>
                                                </v-col>
                                            </v-row>

                                        </div>
                                    </v-col>
                                    <v-divider></v-divider>
                                    <v-spacer></v-spacer>
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Commentaire de l'agent</h4>
                                            <vue-editor v-model="editedItem.commentaires"></vue-editor>

                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col cols="12">
                                            <h4>Votre commentaire</h4>
                                            <!-- <v-textarea prepend-inner-icon="mdi-format-align-justify" 
                               v-model="editedItem.commentaireAdmin" label="Commentaires de l'admin">
                            </v-textarea> -->
                                            <vue-editor v-model="editedItem.commentaireAdmin" label="Commentaires de l'admin">
                                            </vue-editor>

                                        </v-col>
                                        <v-row v-if="editedItem.commentaireAdmin != '' && editedItem.commentedBy != '' && editedItem.commentedBy != null" style="color: red; font-weight:bold">
                                            Ce commentaire a été laissé par {{ editedItem.roleCommentateur }} {{ editedItem.commentedBy
                        }}
                                            à {{ editedItem.dateComment }}.
                                        </v-row>
                                    </v-row>

                                    <br>
                                    <div v-if="editedItem.imagesJoined != null && editedItem.imagesJoined.length > 0">
                                        <v-divider></v-divider>
                                        <h4> Images envoyées </h4>
                                        <v-row>
                                            <div v-for="item of editedItem.imagesJoined" v-bind:key="item.cle" v-bind:video="item">
                                                <v-avatar size="100px" class="my-2" @click="() => openLink(item)">
                                                    <img :src="item" />
                                                </v-avatar>
                                                &nbsp;&nbsp;
                                            </div>

                                        </v-row>
                                    </div>
                                    <br>
                                    <div v-if="editedItem.documentsJoined != null && editedItem.documentsJoined.length > 0">
                                        <v-divider></v-divider>
                                        <h4> Documents envoyées</h4>
                                        <v-row>
                                            <div v-for="item of editedItem.documentsJoined" v-bind:key="item.cle" v-bind:video="item">
                                                <v-btn class="ma-2" color="error" @click="() => openLink(item)" download>
                                                    <b> VOIR LE FICHIER {{ editedItem.documentsJoined.indexOf(item) + 1 }}</b>
                                                </v-btn>
                                                &nbsp;&nbsp;
                                            </div>
                                        </v-row>
                                    </div>

                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12" class="text-right">
                                    <v-btn :disabled="loading" class="success" color="" text @click="save">Enregistrer</v-btn>
                                    <v-btn :disabled="loading" color="" text @click="closeView">Quitter</v-btn>
                                </v-col>
                            </v-row>
                        </v-form>
                    </base-material-card>
                </v-dialog>

                <v-dialog v-model="dialogMail" max-width="450px">
                    <base-material-card>
                        <template v-slot:heading>
                            <div class="display-2 font-weight-light">
                                Envoyer le formulaire par email
                            </div>

                            <div class="subtitle-1 font-weight-light">
                                Completer les informations
                            </div>
                        </template>
                        <v-form v-bind:disabled="loading" lazy-validation ref="dialogForm">
                            <v-row>
                                <v-col cols="12">
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field prepend-inner-icon="mdi-email-outline" v-model="email" label="Email du destinataire" :rules="titleRule"></v-text-field>
                                        </v-col>
                                    </v-row>
                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12" class="text-right">
                                    <v-btn :disabled="loading" class="success" color="" text @click="sendMail">Enregistrer</v-btn>
                                    <v-btn :disabled="loading" color="" text @click="closeMail">Quitter</v-btn>
                                </v-col>
                            </v-row>
                        </v-form>
                    </base-material-card>
                </v-dialog>
                <v-spacer></v-spacer>
                <v-btn :disabled="loading1" class="mb-2 white--text py-4" @click="addItem()" color="rgba(0, 157, 160, 0.11)" depressed dark small v-bind="attrs" v-on="on">
                    <v-icon color="rgb(0, 144, 146)">
                        mdi-plus
                    </v-icon>
                </v-btn>

                <v-btn text icon class="mb-2 ml-2" @click="initialize">
                    <v-icon>mdi-refresh</v-icon>
                </v-btn>

            </v-toolbar>
            <v-divider class="mx-4" inset horizontal />
            <v-card-title>
                <v-spacer></v-spacer>

                <v-col class="d-flex align-baseline" cols="12" sm="6" md="4">
                    <v-text-field v-model="search" append-icon="mdi-magnify" label="Rechercher ici" single-line hide-details></v-text-field>

                    <v-btn outlined class="mx-2 py-4" dark small color="red" :loading="loading2" :disabled="loading2" @click="exportPDF()">
                        <v-icon color="red">
                            mdi-file-pdf-box
                        </v-icon>
                    </v-btn>

                    <v-btn outlined class="mx-2 py-4" dark small color="green" :loading="loading1" :disabled="loading1" @click="exportToExcel()">
                        <v-icon color="green">
                            mdi-cloud-upload
                        </v-icon>
                    </v-btn>
                </v-col>

            </v-card-title>
        </template>

        <template v-slot:[`item.status`]="{ value }">
            <v-chip v-if="value == 'En cours'" color="green" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
            <v-chip v-if="value == 'Validé'" color="blue" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
            <v-chip v-if="value == 'Non validé'" color="red" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
            <v-chip v-if="value == 'Non approuvé'" color="red" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
            <v-chip v-if="value == 'Approuvé'" color="blue" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
            <v-chip v-if="value != 'Approuvé' && value != 'Non approuvé' && value != 'Validé' && value != 'Non validé' && value != 'En cours'" color="grey" class="ma-2" outlined style="color: aliceblue">{{ value }}</v-chip>
        </template>
        <template v-slot:[`item.documentAccompagne`]="{ value }">
            <v-btn v-if="value != ''" class="ma-2" color="error" @click="() => openLink(value)" download>
                VOIR
            </v-btn>
        </template>
        <template v-slot:[`item.commentaireAdmin`]="{ value }">
            <p>
                {{ value }}
            </p>
        </template>

        <template v-slot:[`item.idForm`]="{ item }">
            <v-chip color="blue" style="color: aliceblue" class="ma-2" outlined @click="viewItem(item)">{{ item.idForm }}
            </v-chip>
        </template>

        <template v-slot:[`item.actions`]="{ item }">

            <v-menu bottom left>
                <template v-slot:activator="{ on, attrs }">

                    <v-btn class="mx-2 " outlined fab dark small color="black" v-bind="attrs" v-on="on">
                        <v-icon color="black">
                            mdi-dots-vertical
                        </v-icon>
                    </v-btn>

                </template>

                <v-list>
                    <v-list-item link>
                        <v-list-item-title @click="exportFormToPDF(item.idForm)">Exporter</v-list-item-title>
                    </v-list-item>
                    <v-list-item link>
                        <v-list-item-title @click="printForm(item.idForm)">Imprimer</v-list-item-title>
                    </v-list-item>
                    <v-list-item link>
                        <v-list-item-title @click="viewItem(item)">Editer</v-list-item-title>
                    </v-list-item>
                    <v-list-item link>
                        <v-list-item-title @click="viewMail(item)">Envoyer</v-list-item-title>
                    </v-list-item>
                    <v-list-item link>
                        <v-list-item-title @click="deleteItem(item)">Supprimer</v-list-item-title>
                    </v-list-item>
                </v-list>
            </v-menu>

        </template>

        <template v-slot:no-data>
            <v-btn color="primary" @click="initialize">Réinitialiser</v-btn>
        </template>
    </v-data-table>

    <v-snackbar v-model="snack" :timeout="3000" :color="snackColor">
        {{ snackText }}
        <template v-slot:action="{ attrs }">
            <v-btn v-bind="attrs" text @click="snack = false">Fermer</v-btn>
        </template>
    </v-snackbar>
</v-card>
</template>

<script>
import {
    mapActions,
    mapGetters
} from "vuex";
const Excel = require("exceljs");
import {
    firestore
} from "firebase";
import * as fs from "file-saver";
import {
    storage
} from "firebase";
import jsPDF from "jspdf";
import {
    VueEditor
} from "vue2-editor";
import AXIOS from "axios";

export default {
    components: {
        VueEditor
    },
    data() {
        return {

            snack: false,
            snackColor: "",
            snackText: "",
            search: "",
            loading: true,
            loading1: false,
            loading2: false,
            dialogAdd: false,
            dialog: false,
            dialogView: false,
            dialogMail: false,
            email: "",
            formId: "",
            headers: [{
                    text: "ID",
                    value: "idForm"
                },
                {
                    text: "Titre du contrôle",
                    value: "titleControleVehi"
                },
                {
                    text: "Nom et prénoms",
                    align: "start",
                    sortable: true,
                    value: "userName",
                },
                {
                    text: "Date et heure d'envoi",
                    value: "dateEnvoi"
                },
                // { text: "Commentaires", value: "commentaireAdmin" },
                {
                    text: "Statut",
                    value: "status",
                    sortable: true
                },
                {
                    text: "Actions",
                    value: "actions",
                    sortable: false
                },
            ],
            titleRule: [(v) => !!v || "Champ obligatoire"],
            items3: ["Oui", "Non"],
            items2: localStorage.getItem("role") == "Superviseur (Chef Service)" ? ["Terminé", "Validé", "Non validé", "En cours"] : ["Terminé", "Ouvert", "Ouvert", "Approuvé", "Refusé", "Réglé", "Non réglé(pièces en commande)", "Non réglé (matériel à changer)", "Non réglé (pas de contrat)", "En cours"],
            editedIndex: -1,
            editedItem: {
                detailStatut: "",
                societe: '',
                numeroEngin: '',
                commentaires: "",
                roleCommentateur: "",
                commentedBy: "",
                titleControleVehi: "",
                imagesJoined: "",
                documentsJoined: "",
                titleVehicule1: "",
                immatriculation: "",
                Qimmatriculation1: "",
                macaron: "",
                permisCivile: "",
                permisAtAm: "",
                carteGrise: "",
                visiteTech: "",
                assurance: "",
                guideSecurite: "",
                planQuadrille: "",
                logo: "",
                gyrophare: "",
                radio: "",
                extincteur: "",
                boitePharmacie: "",
                triangleSecu: "",
                // titleVehicule2: "",
                // immatriculation2: "",
                // Qimmatriculation2: "",
                // macaron2: "",
                // permisCivile2: "",
                // permisAtAm2: "",
                // carteGrise2: "",
                // visiteTech2: "",
                // assurance2: "",
                // guideSecurite2: "",
                // planQuadrille2: "",
                // logo2: "",
                // gyrophare2: "",
                // radio2: "",
                // extincteur2: "",
                // boitePharmacie2: "",
                // triangleSecu2: "",
                userId: "",
                userName: "",
                status: "",
                commentaireAdmin: "",
                dateEnvoi: "",
                id: "",
            },
            defaultItem: {
                detailStatut: "",
                societe: '',
                numeroEngin: '',
                commentaires: "",
                roleCommentateur: "",
                commentedBy: "",
                titleControleVehi: "",
                titleVehicule2: "",
                titleVehicule1: "",
                imagesJoined: [],
                documentsJoined: [],
                immatriculation: "",
                Qimmatriculation1: "",
                macaron: "",
                permisCivile: "",
                permisAtAm: "",
                carteGrise: "",
                visiteTech: "",
                assurance: "",
                guideSecurite: "",
                planQuadrille: "",
                logo: "",
                gyrophare: "",
                radio: "",
                extincteur: "",
                boitePharmacie: "",
                triangleSecu: "",
                // immatriculation2: "",
                // Qimmatriculation2: "",
                // macaron2: "",
                // permisCivile2: "",
                // permisAtAm2: "",
                // carteGrise2: "",
                // visiteTech2: "",
                // assurance2: "",
                // guideSecurite2: "",
                // planQuadrille2: "",
                // logo2: "",
                // gyrophare2: "",
                // radio2: "",
                // extincteur2: "",
                // boitePharmacie2: "",
                // triangleSecu2: "",
                userId: "",
                userName: "",
                status: "En cours",
                commentaireAdmin: "",
                dateEnvoi: "",
                id: "",
            },
            defaultItemInit: {
                detailStatut: "",
                societe: '',
                numeroEngin: '',
                commentaires: "",
                roleCommentateur: "",
                commentedBy: "",
                titleControleVehi: "",
                titleVehicule2: "",
                titleVehicule1: "",
                imagesJoined: [],
                documentsJoined: [],
                immatriculation: "",
                Qimmatriculation1: "",
                macaron: "",
                permisCivile: "",
                permisAtAm: "",
                carteGrise: "",
                visiteTech: "",
                assurance: "",
                guideSecurite: "",
                planQuadrille: "",
                logo: "",
                gyrophare: "",
                radio: "",
                extincteur: "",
                boitePharmacie: "",
                triangleSecu: "",
                // immatriculation2: "",
                // Qimmatriculation2: "",
                // macaron2: "",
                // permisCivile2: "",
                // permisAtAm2: "",
                // carteGrise2: "",
                // visiteTech2: "",
                // assurance2: "",
                // guideSecurite2: "",
                // planQuadrille2: "",
                // logo2: "",
                // gyrophare2: "",
                // radio2: "",
                // extincteur2: "",
                // boitePharmacie2: "",
                // triangleSecu2: "",
                userId: "",
                userName: "",
                status: "En cours",
                commentaireAdmin: "",
                dateEnvoi: "",
                id: "",
            },
        };
    },
    computed: {
        ...mapGetters({
            controlesVehi: "controlesVehi/getcontrolesVehi",
        }),
        formTitle() {
            return "Modifier le statut";
        },
    },

    watch: {
        dialog(val) {
            val || this.close();
        },
        dialogView(val) {
            val || this.close();
        },
    },

    created() {
        this.initialize();
    },

    methods: {
        //mail sending//
        viewMail(item) {
            this.editedIndex = this.controlesVehi.indexOf(item);
            this.editedItem = Object.assign({}, item);
            this.formId = this.editedItem.idForm;
            this.dialogMail = true;
        },
        closeMail() {
            this.dialogMail = false;
            this.$nextTick(() => {
                this.editedItem = Object.assign({}, this.defaultItem);
                this.editedIndex = -1;
            });
        },
        async sendMail() {
            if (!this.$refs.dialogForm.validate()) return;
            this.loading = true;
            let Columns = [{
                    header: "Titre du contrôle",
                    key: "titleControleVehi",
                }, {
                    header: "Statut",
                    key: "status"
                }, {
                    header: "Adresse email de l'agent",
                    key: "userEmail"
                },
                {
                    header: "Date et heure d'envoi",
                    key: "dateEnvoi"
                },
                {
                    header: "Agent",
                    key: "userName",
                },
                {
                    header: "Société",
                    key: "societe",
                }, {
                    header: "Numéro de l'engin",
                    key: "numeroEngin",
                },

                {
                    header: "Immatriculation",
                    key: "immatriculation"
                },
                {
                    header: "Immatriculation ?",
                    key: "Qimmatriculation1"
                },
                {
                    header: "Macaron",
                    key: "macaron",
                },
                {
                    header: "Permis civile",
                    key: "permisCivile"
                },
                {
                    header: "Permis « AT » / « AM »",
                    key: "permisAtAm",
                },
                {
                    header: "Carte grise",
                    key: "carteGrise",
                },
                {
                    header: "Visite technique",
                    key: "visiteTech",
                },
                {
                    header: "Assurance",
                    key: "assurance",
                },
                {
                    header: "Guide de sécurité",
                    key: "guideSecurite",
                },
                {
                    header: "Plan quadrillé",
                    key: "planQuadrille",
                },
                {
                    header: "Logo",
                    key: "logo",
                },
                {
                    header: "Gyrophare",
                    key: "gyrophare",
                },
                {
                    header: "Équipement radio",
                    key: "radio",
                },
                {
                    header: "Extincteur",
                    key: "extincteur",
                },
                {
                    header: "Boite à pharmacie",
                    key: "boitePharmacie"
                },
                {
                    header: "Triangles de sécurité /cônes fluorescentes",
                    key: "triangleSecu",
                },

                // {
                //   header: "Observations sur le véhicule 1",
                //   key: "observationVehicule1",
                // },
                // {
                //   header: "Véhicule 2",
                //   key: "titleVehicule2",
                // },
                // { header: "Immatriculation", key: "immatriculation2" },
                // { header: "Immatriculation ?", key: "Qimmatriculation2" },
                // {
                //   header: "Macaron",
                //   key: "macaron2",
                // },
                // { header: "Permis civile", key: "permisCivile2" },
                // {
                //   header: "Permis « AT » / « AM »",
                //   key: "permisAtAm2",
                // },
                // {
                //   header: "Carte grise",
                //   key: "carteGrise2",
                // },
                // {
                //   header: "Visite technique",
                //   key: "visiteTech2",
                // },
                // {
                //   header: "Assurance",
                //   key: "assurance2",
                // },
                // {
                //   header: "Guide de sécurité",
                //   key: "guideSecurite2",
                // },
                // {
                //   header: "Plan quadrillé",
                //   key: "planQuadrille2",
                // },
                // {
                //   header: "Logo",
                //   key: "logo2",
                // },
                // {
                //   header: "Gyrophare",
                //   key: "gyrophare2",
                // },
                // {
                //   header: "Équipement radio",
                //   key: "radio2",
                // },
                // {
                //   header: "Extincteur",
                //   key: "extincteur2",
                // },
                // { header: "Boite à pharmacie", key: "boitePharmacie2" },
                // {
                //   header: "Triangles de sécurité /cônes fluorescentes",
                //   key: "triangleSecu2",
                // },

                // {
                //   header: "Observations sur le véhicule 2",
                //   key: "observationVehicule2",
                // },
            ];
            this.loading = true;
            var m = new Date();
            var dateString =
                ("0" + m.getUTCDate()).slice(-2) + "-" +
                ("0" + (m.getUTCMonth() + 1)).slice(-2) + "-" +
                m.getUTCFullYear() + " " +
                ("0" + m.getUTCHours()).slice(-2) + ":" +
                ("0" + m.getUTCMinutes()).slice(-2);
            const snapshot = await firestore().collection("controleVehicule").where("idForm", "==", this.formId).get();
            const documents = snapshot.docs.map((doc) => doc.data());
            const doc = new jsPDF();
            doc.addImage("data:image/png;base64,", 'JPEG', 20, 0, 25, 25)
            doc.setFontSize(16);
            doc.setFont("helvetica");
            doc.setTextColor(255, 0, 0);
            let pageHeight = doc.internal.pageSize.height;
            doc.text("Contrôle de véhicules (Code: " + this.formId + ")", 105, 15, null, null, "center");
            doc.setTextColor(0, 2, 3);
            let i = 35;
            Columns.forEach((element) => {
                doc.text(element["header"] + " : " + documents[0][element["key"]], 10, i, {
                    align: "left",
                });
                if (i >= pageHeight) {
                    doc.addPage();
                    i = 15
                } else {
                    i = i + 10;
                }
            });
            doc.setTextColor(51, 10, 199);
            doc.text("Ce document a été généré par le système GES-UPRA au " + dateString, 105, pageHeight - 5, {
                align: "center",
            });
            var pdfBase64 = doc.output('datauristring');

            let config = {
                headers: {
                    'Content-Type': 'application/json',
                    "Authorization": "key=AAAALeem-1k:APA91bFi6XX1XOe2gz-VGRBEKI00LfWbXS8TpYRQKzKzc9L3mVKJgZwXWJuNZetn5F316V1ePT_qwFyrR-6q7nwVGs7Hp649JGlEb-wUdpRfDyfvrEcaUxFlsGEjjMXjZY6ap2pQwY-Q"
                }
            };
            let body = {
                pdf: pdfBase64,
                formId: this.formId,
                email: this.email,
            };
            await AXIOS.post("https://thawing-badlands-05581.herokuapp.com/sendMail", body, config);
            this.loading = false;
            this.snack = true;
            this.snackColor = "success";
            this.snackText = "Mail envoyé avec succès.";
            this.dialogMail = false;
        },

        //send mail to user finish//
        async deleteItem(item) {
            this.loading = true;
            if (confirm("Etes vous sûr(e) de vouloir supprimer ce formulaire?")) {
                this.loading = true;
                try {
                    await this.removecontrolesVehi(item);
                    this.loading = false;
                    this.snack = true;
                    this.snackColor = "success";
                    this.snackText = "Formulaire supprimé avec succès.";
                } catch (e) {
                    this.loading = false;
                    this.snack = true;
                    this.snackColor = "error";
                    this.snackText = "Formulaire non supprimé";
                    console.error(e);
                }
            } else {
                this.loading = false;
            }
        },

        async exportFormToPDF(idForm) {
            let Columns = [{
                    header: "Titre du contrôle",
                    key: "titleControleVehi",
                }, {
                    header: "Statut",
                    key: "status"
                }, {
                    header: "Adresse email de l'agent",
                    key: "userEmail"
                },
                {
                    header: "Date et heure d'envoi",
                    key: "dateEnvoi"
                },
                {
                    header: "Agent",
                    key: "userName",
                },
                {
                    header: "Société",
                    key: "societe",
                }, {
                    header: "Numéro de l'engin",
                    key: "numeroEngin",
                },

                {
                    header: "Immatriculation",
                    key: "immatriculation"
                },
                {
                    header: "Immatriculation ?",
                    key: "Qimmatriculation1"
                },
                {
                    header: "Macaron",
                    key: "macaron",
                },
                {
                    header: "Permis civile",
                    key: "permisCivile"
                },
                {
                    header: "Permis « AT » / « AM »",
                    key: "permisAtAm",
                },
                {
                    header: "Carte grise",
                    key: "carteGrise",
                },
                {
                    header: "Visite technique",
                    key: "visiteTech",
                },
                {
                    header: "Assurance",
                    key: "assurance",
                },
                {
                    header: "Guide de sécurité",
                    key: "guideSecurite",
                },
                {
                    header: "Plan quadrillé",
                    key: "planQuadrille",
                },
                {
                    header: "Logo",
                    key: "logo",
                },
                {
                    header: "Gyrophare",
                    key: "gyrophare",
                },
                {
                    header: "Équipement radio",
                    key: "radio",
                },
                {
                    header: "Extincteur",
                    key: "extincteur",
                },
                {
                    header: "Boite à pharmacie",
                    key: "boitePharmacie"
                },
                {
                    header: "Triangles de sécurité /cônes fluorescentes",
                    key: "triangleSecu",
                },

                // {
                //   header: "Observations sur le véhicule 1",
                //   key: "observationVehicule1",
                // },

                // { header: "Commentaires de l'administrateur", key: "commentaireAdmin" },

            ];
            this.loading = true;
            var m = new Date();
            var dateString =
                ("0" + m.getUTCDate()).slice(-2) + "-" +
                ("0" + (m.getUTCMonth() + 1)).slice(-2) + "-" +
                m.getUTCFullYear() + " " +
                ("0" + m.getUTCHours()).slice(-2) + ":" +
                ("0" + m.getUTCMinutes()).slice(-2);
            const snapshot = await firestore().collection("controleVehicule").where("idForm", "==", idForm).get();
            const documents = snapshot.docs.map((doc) => doc.data());
            const doc = new jsPDF();
            doc.addImage("data:image/png;base64,", 'JPEG', 20, 0, 25, 25)
            doc.setFontSize(16);
            doc.setFont("helvetica");
            doc.setTextColor(255, 0, 0);
            let pageHeight = doc.internal.pageSize.height;
            doc.text("Contrôle de véhicules (Code: " + idForm + ")", 105, 15, null, null, "center");
            doc.setTextColor(0, 2, 3);
            let i = 35;
            Columns.forEach((element) => {
                doc.text(element["header"] + " : " + documents[0][element["key"]], 10, i, {
                    align: "left",
                });
                if (i >= pageHeight) {
                    doc.addPage();
                    i = 15
                } else {
                    i = i + 10;
                }
            });
            doc.setTextColor(51, 10, 199);
            doc.text("Ce document a été généré par le système GES-UPRA au " + dateString, 105, pageHeight - 5, {
                align: "center",
            });
            doc.save("UPRA FORM (" + idForm + ").pdf");
            this.loading = false;
        },

        async printForm(idForm) {
            let Columns = [{
                    header: "Titre du contrôle",
                    key: "titleControleVehi",
                }, {
                    header: "Statut",
                    key: "status"
                }, {
                    header: "Adresse email de l'agent",
                    key: "userEmail"
                },
                {
                    header: "Date et heure d'envoi",
                    key: "dateEnvoi"
                },
                {
                    header: "Agent",
                    key: "userName",
                },
                {
                    header: "Société",
                    key: "societe",
                }, {
                    header: "Numéro de l'engin",
                    key: "numeroEngin",
                },

                {
                    header: "Immatriculation",
                    key: "immatriculation"
                },
                {
                    header: "Immatriculation ?",
                    key: "Qimmatriculation1"
                },
                {
                    header: "Macaron",
                    key: "macaron",
                },
                {
                    header: "Permis civile",
                    key: "permisCivile"
                },
                {
                    header: "Permis « AT » / « AM »",
                    key: "permisAtAm",
                },
                {
                    header: "Carte grise",
                    key: "carteGrise",
                },
                {
                    header: "Visite technique",
                    key: "visiteTech",
                },
                {
                    header: "Assurance",
                    key: "assurance",
                },
                {
                    header: "Guide de sécurité",
                    key: "guideSecurite",
                },
                {
                    header: "Plan quadrillé",
                    key: "planQuadrille",
                },
                {
                    header: "Logo",
                    key: "logo",
                },
                {
                    header: "Gyrophare",
                    key: "gyrophare",
                },
                {
                    header: "Équipement radio",
                    key: "radio",
                },
                {
                    header: "Extincteur",
                    key: "extincteur",
                },
                {
                    header: "Boite à pharmacie",
                    key: "boitePharmacie"
                },
                {
                    header: "Triangles de sécurité /cônes fluorescentes",
                    key: "triangleSecu",
                },

                // {
                //   header: "Observations sur le véhicule 1",
                //   key: "observationVehicule1",
                // },
            ];
            this.loading = true;
            var m = new Date();
            var dateString =
                ("0" + m.getUTCDate()).slice(-2) + "-" +
                ("0" + (m.getUTCMonth() + 1)).slice(-2) + "-" +
                m.getUTCFullYear() + " " +
                ("0" + m.getUTCHours()).slice(-2) + ":" +
                ("0" + m.getUTCMinutes()).slice(-2);
            const snapshot = await firestore().collection("controleVehicule").where("idForm", "==", idForm).get();
            const documents = snapshot.docs.map((doc) => doc.data());
            const doc = new jsPDF();
            doc.addImage("data:image/png;base64,", 'JPEG', 20, 0, 25, 25)
            doc.setFontSize(16);
            doc.setFont("helvetica");
            doc.setTextColor(255, 0, 0);
            let pageHeight = doc.internal.pageSize.height;
            doc.text("Contrôle de véhicules (Code: " + idForm + ")", 105, 15, null, null, "center");
            doc.setTextColor(0, 2, 3);
            let i = 35;
            Columns.forEach((element) => {
                doc.text(element["header"] + " : " + documents[0][element["key"]], 10, i, {
                    align: "left",
                });
                if (i >= pageHeight) {
                    doc.addPage();
                    i = 15
                } else {
                    i = i + 10;
                }
            });
            doc.setTextColor(51, 10, 199);
            doc.text("Ce document a été généré par le système GES-UPRA au " + dateString, 105, pageHeight - 5, {
                align: "center",
            });
            doc.autoPrint({
                variant: 'non-conform'
            });
            doc.output('dataurlnewwindow');
            this.loading = false;
        },

        ...mapActions({
            loadcontrolesVehi: "controlesVehi/loadcontrolesVehi",
        }),
        openLink(link) {
            window.open(link, "_blank");
        },
        ...mapActions({
            addcontroleVehi: "controlesVehi/addcontroleVehi",
            updatecontrolesVehi: "controlesVehi/updatecontrolesVehi",
            removecontrolesVehi: "controlesVehi/removecontrolesVehi",
        }),
        getcontrolesVehiById: function (id) {
            let controlesVehi = this.controlesVehi.filter((c) => c.id == id)[0];
            console.log(controlesVehi.status);
            if (controlesVehi === undefined) {
                console.log("undefined qarşim");
                return {
                    nom: "Unknown"
                };
            }
            return controlesVehi;
        },
        //To upload
        async uploadTaskImages() {
            this.snack = true;
            this.snackColor = "danger";
            this.snackText = this.$refs.uploader.files.length + " images prises en compte";
            let vm = this;
            vm.defaultItem.imagesJoined = [];
            for (let i = 0; i < this.$refs.uploader.files.length; i++) {
                const newImage = this.$refs.uploader.files[i];
                if (newImage != null && newImage.name) {
                    let ref = storage()
                        .ref()
                        .child("images/" + newImage.name);
                    await ref.put(newImage).then((snapshot) => {
                        if (snapshot.state === "success") {
                            snapshot.ref.getDownloadURL().then(function (downloadURL) {
                                vm.defaultItem.imagesJoined.push(downloadURL);
                            });
                        }
                    });
                }
            }
        },

        async uploadTaskDocuments() {
            this.snack = true;
            this.snackColor = "danger";
            this.snackText = this.$refs.uploader1.files.length + " documents prises en compte";
            let vm = this;
            vm.defaultItem.documentsJoined = [];
            for (let i = 0; i < this.$refs.uploader1.files.length; i++) {
                const newDocument = this.$refs.uploader1.files[i];
                if (newDocument != null && newDocument.name) {
                    let ref = storage()
                        .ref()
                        .child("documents/" + newDocument.name);
                    await ref.put(newDocument).then((snapshot) => {
                        if (snapshot.state === "success") {
                            snapshot.ref.getDownloadURL().then(function (downloadURL) {
                                vm.defaultItem.documentsJoined.push(downloadURL);
                            });
                        }
                    });
                }
            }
        },
        //-------------------------------------------------------------end upload

        exportPDF() {
            this.loading2 = true;
            let exportColumns = [{
                    title: "ID",
                    dataKey: "idForm",
                },
                {
                    title: "Titre",
                    dataKey: "titleControleVehi",
                },
                {
                    title: "Agent",
                    dataKey: "userName",
                },
                {
                    title: "Adresse email",
                    dataKey: "userEmail"
                },
                {
                    title: "Date et heure",
                    dataKey: "dateEnvoi"
                },
                {
                    title: "Immatriculation",
                    dataKey: "immatriculation",
                },
                // {
                //   title: "Société",
                //   datakey: "societe",
                // }, {
                //   title: "Numéro de l'engin",
                //   datakey: "numeroEngin",
                // },

                // {
                //   title: "Observations",
                //   dataKey: "observationVehicule1",
                // },

                // { title: "Commentaires de l'administrateur", dataKey: "commentaireAdmin" },
                {
                    title: "Statut",
                    dataKey: "status"
                },
            ];
            import("jspdf").then(jsPDF => {
                import("jspdf-autotable").then(() => {
                    const doc = new jsPDF.default('l', 'pt', "a4");
                    doc.setFontSize(16);
                    doc.setFont("helvetica");
                    doc.text("Liste de contrôles des véhicules", 30, 30, {
                        align: "justify",
                    });
                    doc.autoTable(exportColumns, this.controlesVehi);
                    doc.save("Contrôle des véhicules.pdf");
                    this.loading2 = false;
                })
            });
        },
        async exportToExcel() {
            this.loading1 = true;
            let workbook = new Excel.Workbook();
            let worksheet = workbook.addWorksheet("Contrôle des véhicules");
            worksheet.columns = [{
                    header: "Titre du contrôle",
                    key: "titleControleVehi",
                }, {
                    header: "Statut",
                    key: "status"
                }, {
                    header: "Adresse email de l'agent",
                    key: "userEmail"
                },
                {
                    header: "Date et heure d'envoi",
                    key: "dateEnvoi"
                },
                {
                    header: "Agent",
                    key: "userName",
                },
                {
                    header: "Société",
                    key: "societe",
                }, {
                    header: "Numéro de l'engin",
                    key: "numeroEngin",
                },

                {
                    header: "Immatriculation",
                    key: "immatriculation"
                },
                {
                    header: "Immatriculation ?",
                    key: "Qimmatriculation1"
                },
                {
                    header: "Macaron",
                    key: "macaron",
                },
                {
                    header: "Permis civile",
                    key: "permisCivile"
                },
                {
                    header: "Permis « AT » / « AM »",
                    key: "permisAtAm",
                },
                {
                    header: "Carte grise",
                    key: "carteGrise",
                },
                {
                    header: "Visite technique",
                    key: "visiteTech",
                },
                {
                    header: "Assurance",
                    key: "assurance",
                },
                {
                    header: "Guide de sécurité",
                    key: "guideSecurite",
                },
                {
                    header: "Plan quadrillé",
                    key: "planQuadrille",
                },
                {
                    header: "Logo",
                    key: "logo",
                },
                {
                    header: "Gyrophare",
                    key: "gyrophare",
                },
                {
                    header: "Équipement radio",
                    key: "radio",
                },
                {
                    header: "Extincteur",
                    key: "extincteur",
                },
                {
                    header: "Boite à pharmacie",
                    key: "boitePharmacie"
                },
                {
                    header: "Triangles de sécurité /cônes fluorescentes",
                    key: "triangleSecu",
                },

                {
                    header: "Observations sur le véhicule 1",
                    key: "observationVehicule1",
                },
                {
                    header: "Statut",
                    key: "status"
                },
            ];
            worksheet.columns.forEach(function (column) {
                var maxLength = 0;
                column["eachCell"]({
                    includeEmpty: true
                }, function (cell) {
                    var columnLength = cell.value ? cell.value.toString().length : 10;
                    if (columnLength > maxLength) {
                        maxLength = columnLength;
                    }
                });
                column.width = maxLength < 10 ? 10 : maxLength;
            });
            worksheet.getRow(1).font = {
                bold: true,
                color: {
                    argb: "0047AB"
                }
            };
            const snapshot = await firestore().collection("controleVehicule").get();
            snapshot.forEach((doc) => {
                let e = doc.data();
                worksheet.addRow({
                    ...e,
                });
            });
            workbook.xlsx.writeBuffer().then((data) => {
                const blob = new Blob([data], {
                    type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                });
                fs.saveAs(blob, `controlesVehicule.xlsx`);
            });
            this.loading1 = false;
        },
        async initialize() {
            this.loading = true;
            try {
                await this.loadcontrolesVehi();
            } catch (e) {
                console.error(e);
            }
            this.loading = false;
        },
        addItem() {
            this.editedIndex = -1;
            this.defaultItem = Object.assign({}, this.defaultItemInit);
            this.dialogAdd = true;
        },
        closeAdd() {
            this.dialogAdd = false;
            this.$nextTick(() => {
                this.editedItem = Object.assign({}, this.defaultItem);
                this.editedIndex = -1;
            });
        },

        editItem(item) {
            this.editedIndex = this.controlesVehi.indexOf(item);
            this.editedItem = Object.assign({}, item);
            this.dialog = true;
        },

        viewItem(item) {
            this.editedIndex = this.controlesVehi.indexOf(item);
            this.editedItem = Object.assign({}, item);
            this.dialogView = true;
        },

        close() {
            this.dialog = false;
            this.$nextTick(() => {
                this.editedItem = Object.assign({}, this.defaultItem);
                this.editedIndex = -1;
            });
        },

        closeView() {
            this.dialogView = false;
            this.$nextTick(() => {
                this.editedItem = Object.assign({}, this.defaultItem);
                this.editedIndex = -1;
            });
        },

        async save() {
            if (!this.$refs.dialogForm.validate()) return;
            this.loading = true;
            if (this.editedIndex <= -1) {
                this.snack = true;
                this.snackColor = "blue";
                this.snackText = "Action en cours...";
                await this.uploadTaskImages();
                await this.uploadTaskDocuments();
            }

            if (this.editedIndex > -1) {
                this.loading = true;
                try {
                    await this.updatecontrolesVehi({
                        index: this.editedIndex,
                        order: this.editedItem,
                    });
                    this.loading = false;
                    this.dialogView = false;
                    this.dialogAdd = false;
                    this.close();
                    this.snack = true;
                    this.snackColor = "success";
                    this.snackText = "Le statut du formulaire a été mise à jour";
                } catch (e) {
                    this.loading = false;
                    this.close();

                    this.snack = true;
                    this.snackColor = "error";
                    this.snackText =
                        "Le statut du formulaire n'a pas pu être mise à jour";
                    console.error(e);
                }
            } else {
                this.loading = true;
                try {
                    await this.addcontroleVehi(this.defaultItem);
                    this.loading = false;
                    this.close();
                    this.dialogAdd = false;
                    this.snack = true
                    this.snackColor = 'success'
                    this.snackText = 'Formulaire ajouté avec succès.'
                } catch (e) {
                    this.loading = false;
                    this.close();
                    this.snack = true
                    this.snackColor = 'error'
                    this.snackText = 'Le formulaire n\'a pas été ajouté.'
                    console.error(e);
                }
            }
        },
    },
    filters: {
        truncate: function (text, length, suffix) {
            if (text.length > length) {
                return text.substring(0, length) + suffix;
            } else {
                return text;
            }
        },
    },
};
</script>
